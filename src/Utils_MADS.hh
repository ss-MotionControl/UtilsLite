...
namespace Utils {
  ...
  public:
     ...
     */
    MADS_minimizer( Options const & o = Options() )
    : m_opts(o)
    {}
  ...
  private:
    ...
    generate_search_points( std::vector<Vector> & search_points ) {
      ...
      for (size_t i = 0; i < m_opts.search_points; ++i) {
        ...
        if ((point - m_current_x).norm() > m_mesh_size * 0.1) {
    ...
    poll_step( Callback const & fun ) {
      ...
      if (m_opts.poll_method == "ortho") {
        generate_orthomads_directions(directions);
      } else {
      ...
      for (auto const& dir : directions) {
        ...
        if ((poll_point - m_current_x).norm() < m_mesh_size * 1e-6) {
          continue;
        }
    ...
    check_convergence( size_t iteration, size_t no_improvement_count ) const {
      ...
      if (m_mesh_size <= m_opts.tol_mesh) {
        return { 
          m_current_f, m_current_x, iteration, m_eval_count, true, m_mesh_size,
          "Mesh size below tolerance" 
        };
      }
  ...
  public:
    ...
    minimize( Vector const & x0, Callback const & fun ) {
      ...
      if ( m_opts.verbose ) {
        fmt::print( 
          "[MADS] Starting optimization, dimension: {}, initial f: {}, initial mesh: {}\n", 
      ...
      for ( size_t k{0}; k < m_opts.max_iter; ++k ) {
        ...
        if (m_opts.enable_search) {
          improvement = search_step(fun);
        }
        
        // POLL STEP (always performed)
        if (!improvement) {
          improvement = poll_step(fun);
        }
        ...
        if ( m_opts.verbose && (k % m_opts.print_every) == 0 ) {
          fmt::print(
            ...
            k, m_current_f, m_mesh_size, improvement, m_eval_count
          );
        }
        ...
        if (m_eval_count >= m_opts.max_evaluations) {
          return { 
            m_current_f, m_current_x, k, m_eval_count, true, m_mesh_size,
            "Maximum function evaluations reached" 
          };
        }
      }
      ...
      return { 
        m_current_f, m_current_x, m_opts.max_iter, m_eval_count, false, m_mesh_size, 
    ...
    get_state_info() const {
      return fmt::format(
        ...
        m_current_f, m_mesh_size, m_eval_count, m_visited_points.size()
      );
    }
