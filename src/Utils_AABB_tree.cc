...
bool AABBtree<Real>::overlap1( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[1] && bbox1[1] >= bbox2[0];
}

template<typename Real>
bool AABBtree<Real>::overlap2( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[2] && bbox1[2] >= bbox2[0] && bbox1[1] <= bbox2[3] &&
        bbox1[3] >= bbox2[1];
}

template<typename Real>
bool AABBtree<Real>::overlap3( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[3] && bbox1[3] >= bbox2[0] && bbox1[1] <= bbox2[4] &&
        bbox1[4] >= bbox2[1] && bbox1[2] <= bbox2[5] && bbox1[5] >= bbox2[2];
}

template<typename Real>
bool AABBtree<Real>::overlap4( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[4] && bbox1[4] >= bbox2[0] && bbox1[1] <= bbox2[5] &&
        bbox1[5] >= bbox2[1] && bbox1[2] <= bbox2[6] && bbox1[6] >= bbox2[2] &&
        bbox1[3] <= bbox2[7] && bbox1[7] >= bbox2[3];
}

template<typename Real>
bool AABBtree<Real>::overlap5( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[5] && bbox1[5] >= bbox2[0] && bbox1[1] <= bbox2[6] &&
        bbox1[6] >= bbox2[1] && bbox1[2] <= bbox2[7] && bbox1[7] >= bbox2[2] &&
        bbox1[3] <= bbox2[8] && bbox1[8] >= bbox2[3] && bbox1[4] <= bbox2[9] &&
        bbox1[9] >= bbox2[4];
}

template<typename Real>
bool AABBtree<Real>::overlap6( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[6] && bbox1[6] >= bbox2[0] && bbox1[1] <= bbox2[7] &&
        bbox1[7] >= bbox2[1] && bbox1[2] <= bbox2[8] && bbox1[8] >= bbox2[2] &&
        bbox1[3] <= bbox2[9] && bbox1[9] >= bbox2[3] && bbox1[4] <= bbox2[10] &&
        bbox1[10] >= bbox2[4] && bbox1[5] <= bbox2[11] && bbox1[11] >= bbox2[5];
}

template<typename Real>
bool AABBtree<Real>::overlap7( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[7] && bbox1[7] >= bbox2[0] && bbox1[1] <= bbox2[8] &&
        bbox1[8] >= bbox2[1] && bbox1[2] <= bbox2[9] && bbox1[9] >= bbox2[2] &&
        bbox1[3] <= bbox2[10] && bbox1[10] >= bbox2[3] && bbox1[4] <= bbox2[11] &&
        bbox1[11] >= bbox2[4] && bbox1[5] <= bbox2[12] && bbox1[12] >= bbox2[5] &&
        bbox1[6] <= bbox2[13] && bbox1[14] >= bbox2[6];
}

template<typename Real>
bool AABBtree<Real>::overlap8( Real const bbox1[], Real const bbox2[], integer )
{
    return bbox1[0] <= bbox2[8] && bbox1[8] >= bbox2[0] && bbox1[1] <= bbox2[9] &&
        bbox1[9] >= bbox2[1] && bbox1[2] <= bbox2[10] && bbox1[10] >= bbox2[2] &&
        bbox1[3] <= bbox2[11] && bbox1[11] >= bbox2[3] && bbox1[4] <= bbox2[12] &&
        bbox1[12] >= bbox2[4] && bbox1[5] <= bbox2[13] && bbox1[13] >= bbox2[5] &&
        bbox1[6] <= bbox2[14] && bbox1[14] >= bbox2[6] && bbox1[7] <= bbox2[15] &&
        bbox1[15] >= bbox2[7];
}

template<typename Real>
bool AABBtree<Real>::pnt_overlap1( Real const pnt[], Real const bb2[], integer )
{
    return pnt[0] <= bb2[1] && pnt[0] >= bb2[0];
}

template<typename Real>
bool AABBtree<Real>::pnt_overlap2( Real const pnt[], Real const bb2[], integer )
{
    return pnt[0] <= bb2[2] && pnt[0] >= bb2[0] && pnt[1] <= bb2[3] && pnt[1] >= bb2[1];
}

template<typename Real>
bool AABBtree<Real>::pnt_overlap3( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::pnt_overlap4( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::pnt_overlap5( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::pnt_overlap6( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::pnt_overlap7( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::pnt_overlap8( Real const pnt[], Real const bbox2[], integer )
{
...
template<typename Real>
bool AABBtree<Real>::check_overlap( Real const bb1[], Real const bb2[], integer dim )
{
    bool overlap{ false };
    integer k{ dim % 4 };
    switch ( k ) {
...
template<typename Real>
bool AABBtree<Real>::check_overlap_with_point( Real const pnt[], Real const bb2[], integer dim )
{
    bool overlap{ false };
    integer k{ dim % 4 };
    switch ( k ) {
    case 1:
        ...
        break;
    case 2:
        overlap = pnt[0] <= bb2[dim + 0] && pnt[0] >= bb2[0] && pnt[1] <= bb2[dim + 1] &&
            pnt[1] >= bb2[1];
        break;
    case 3:
        overlap = pnt[0] <= bb2[dim + 0] && pnt[0] >= bb2[0] && pnt[1] <= bb2[dim + 1] &&
            pnt[1] >= bb2[1] && pnt[2] <= bb2[dim + 2] && pnt[2] >= bb2[2];
        break;
    }
    pnt += k;
    bb2 += k;
...
template<typename Real>
Real AABBtree<Real>::max_bbox_distance( Real const bbox[], Real const pnt[] ) const
{
    Real res = 0;
...
template<typename Real>
void AABBtree<Real>::set_max_num_objects_per_node( integer n )
{
...
template<typename Real>
void AABBtree<Real>::set_bbox_long_edge_ratio( Real ratio )
{
...
template<typename Real>
void AABBtree<Real>::set_bbox_overlap_tolerance( Real tol )
{
...
template<typename Real>
void AABBtree<Real>::set_bbox_min_size_tolerance( Real tol )
{
    UTILS_ASSERT( tol >= 0,
...
template<typename Real>
void AABBtree<Real>::allocate( integer const nbox, integer const dim )
{
...
template<typename Real>
void AABBtree<Real>::add_bboxes(
    Real const bbox_min[], integer ldim0, Real const bbox_max[], integer ldim1 )
{
    ...
    for ( integer i = 0; i < m_num_objects; ++i ) {
        for ( integer j = 0; j < m_dim; ++j ) {
            UTILS_ASSERT(
                bbox_min[j] <= bbox_max[j], "AABBtree::add_bboxes, bad bbox N.{} max < min", i );
        }
...
template<typename Real>
void AABBtree<Real>::replace_bbox( Real const bbox_min[], Real const bbox_max[], integer ipos )
{
    ...
    for ( integer j = 0; j < m_dim; ++j ) {
        UTILS_ASSERT(
            bbox_min[j] <= bbox_max[j], "AABBtree::replace_bbox, bad bbox N.{} max < min", ipos );
    }
    std::copy_n( bbox_min, m_dim, bb );
...
void AABBtree<Real>::build()
{
    ...
    while ( !m_stack.empty() ) {
        ...
        m_stack.pop_back();
        UTILS_ASSERT_DEBUG( id_father < m_nmax,
        ...
        while ( n_long + n_short < num ) {
            ...
            UTILS_ASSERT_DEBUG( id < m_num_objects,
        ...
        UTILS_ASSERT_DEBUG( id_right < m_nmax,
...
template<typename Real>
void AABBtree<Real>::intersect_with_one_point( Real const pnt[], AABB_SET& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::intersect_with_one_point_and_refine(
    Real const pnt[], AABB_SET& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::intersect_with_one_bbox( Real const bbox[], AABB_SET& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::intersect_with_one_bbox_and_refine(
    Real const bbox[], AABB_SET& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::intersect( AABBtree const& aabb, AABB_MAP& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::intersect_and_refine( AABBtree const& aabb, AABB_MAP& bb_index ) const
{
    m_num_check = 0;
...
template<typename Real>
void AABBtree<Real>::pnt_bbox_minmax(
    Real const pnt[], Real const bbox[], Real& dmin2, Real& dmax2 ) const
{
...
template<typename Real>
void AABBtree<Real>::min_distance_candidates( Real const pnt[], AABB_SET& bb_index ) const
{
...
template<typename Real>
void AABBtree<Real>::get_bbox_indexes_of_a_node( integer i_pos, AABB_SET& bb_index ) const
{
...
template<typename Real>
typename AABBtree<Real>::integer AABBtree<Real>::num_tree_nodes( integer const nmin ) const
{
    integer n{ 0 };
    for ( integer i{ 0 }; i < m_num_tree_nodes; ++i )
...
template<typename Real>
void AABBtree<Real>::get_root_bbox( Real bb_min[], Real bb_max[] ) const
{
...
template<typename Real>
void AABBtree<Real>::get_bboxes_of_the_tree(
    Real bbox_min[], integer ldim0, Real bbox_max[], integer ldim1, integer nmin ) const
{
    UTILS_ASSERT( ldim0 >= m_dim && ldim1 >= m_dim,
