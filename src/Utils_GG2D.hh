...
namespace Utils {
  ...
  public:
    ...
    Real distance(Point2D<Real> const & other) const noexcept {
      return (*this - other).norm();
    }

    //! Squared distance to another point (faster, avoids sqrt)
    Real distance_squared(Point2D<Real> const & other) const noexcept {
      return (*this - other).squaredNorm();
    }

    //! Dot product with another point
    Real dot(Point2D<Real> const & other) const noexcept {
      return this->x() * other.x() + this->y() * other.y();
    }

    //! Cross product with another point (returns scalar z-component)
    Real cross(Point2D<Real> const & other) const noexcept {
    ...
    Point2D<Real> normalized() const {
      Real len = this->norm();
      if (len < machine_eps<Real>()) {
        return Point2D<Real>(0, 0);
      }
    ...
    Point2D<Real> scale(Real sx, Real sy) const noexcept {
    ...
    bool isApprox(Point2D<Real> const & other, Real prec = machine_eps<Real>()) const noexcept {
      return std::abs(this->x() - other.x()) <= prec && 
             std::abs(this->y() - other.y()) <= prec;
    }
  ...
  //! and maximum corners of the box respectively.
  //!
  ...
  public:
    ...
    Box2D(Point2D<Real> const & pmin, Point2D<Real> const & pmax) noexcept
    : m_Pmin(pmin)
    , m_Pmax(pmax)
    {}

    //! Constructor from coordinates
    Box2D(Real xmin, Real ymin, Real xmax, Real ymax) noexcept
    : m_Pmin(xmin, ymin)
    , m_Pmax(xmax, ymax)
    {}

    //! Constructor from a single point
    explicit Box2D(Point2D<Real> const & p) noexcept
    : m_Pmin(p)
    , m_Pmax(p)
    {}
    ...
    void set(Real xmin, Real ymin, Real xmax, Real ymax) noexcept {
      m_Pmin.x(xmin); m_Pmin.y(ymin);
      m_Pmax.x(xmax); m_Pmax.y(ymax);
    }
    ...
    Point2D<Real> center() const noexcept {
      return Point2D<Real>((m_Pmin.x() + m_Pmax.x()) * Real(0.5),
                          (m_Pmin.y() + m_Pmax.y()) * Real(0.5));
    }

    //! Get size as a point (width, height)
    Point2D<Real> size() const noexcept {
    ...
    bool intersects(Box2D<Real> const & other) const noexcept {
      return !(m_Pmax.x() < other.m_Pmin.x() || 
               m_Pmin.x() > other.m_Pmax.x() ||
               m_Pmax.y() < other.m_Pmin.y() || 
    ...
    Box2D<Real> intersection(Box2D<Real> const & other) const {
      ...
      // Ensure valid box
      if (result.m_Pmin.x() > result.m_Pmax.x() || 
          result.m_Pmin.y() > result.m_Pmax.y()) {
        // Return empty box
    ...
    std::array<Point2D<Real>, 4> corners() const noexcept {
      return {{
        m_Pmin,
        Point2D<Real>(m_Pmax.x(), m_Pmin.y()),
        m_Pmax,
        Point2D<Real>(m_Pmin.x(), m_Pmax.y())
      }};
    }
    ...
    bool isApprox(Box2D<Real> const & other, Real prec = machine_eps<Real>()) const noexcept {
      return m_Pmin.isApprox(other.m_Pmin, prec) && 
             m_Pmax.isApprox(other.m_Pmax, prec);
    }

    //! Stream output operator
    friend std::ostream& operator<<(std::ostream& os, Box2D<Real> const & box) {
      os << "Box[min=" << box.Pmin() << ", max=" << box.Pmax() << 
            ", size=" << box.size() << "]";
      return os;
    }
  };
  ...
  public:
    ...
    //! Copy constructor
    Segment2D(Segment2D<Real> const & S) noexcept
    : m_Pa(S.m_Pa)
    , m_Pb(S.m_Pb)
    {}

    //! Constructor from two points
    Segment2D(Point2D<Real> const & A, Point2D<Real> const & B) noexcept
    : m_Pa(A)
    , m_Pb(B)
    {}

    //! Constructor from coordinates
    Segment2D(Real ax, Real ay, Real bx, Real by) noexcept
    : m_Pa(ax, ay)
    , m_Pb(bx, by)
    {}

    //! Move constructor
    Segment2D(Segment2D<Real> && S) noexcept
    ...
    bool is_valid(Real tol = DEFAULT_TOLERANCE) const noexcept {
    ...
    Point2D<Real> direction() const noexcept {
      return m_Pb - m_Pa;
    }
    ...
    Point2D<Real> normal() const noexcept {
    ...
    //! \param s The parameter along the segment where the projection occurs (output).
    //!
    //! \return `true` if the projection lies within [0,1] (on the segment), `false` otherwise.
    //!
    ...
    Point2D<Real> projection(Point2D<Real> const & P, Real & s, Real & t) const {
      ...
      if (norm_len > machine_eps<Real>()) {
        t = (P - PP).dot(N) / norm_len;
      } else {
        t = 0;
      }
    ...
    //!
    Point2D<Real> eval(Real s) const noexcept {
    ...
    Point2D<Real> midpoint() const noexcept {
    ...
    //!
    bool intersect(Segment2D<Real> const & S, Real & s, Real & t, 
                   Real tol = DEFAULT_TOLERANCE) const {
      s = t = 0;
      ...
      if (std::abs(normalized_cross) <= tol) {
        ...
        if (S.projection(m_Pa, t)) { s = 0; return true; }
        if (S.projection(m_Pb, t)) { s = 1; return true; }
        s = t = 0;
        return false;
      }
    ...
    bool intersection_point(Segment2D<Real> const & S, 
                           Point2D<Real> & point,
                           Real tol = DEFAULT_TOLERANCE) const {
      Real s, t;
    ...
    Segment2D<Real> reversed() const noexcept {
  ...
  public:
    ...
    Triangle2D(Point2D<Real> const & A, 
               Point2D<Real> const & B, 
               Point2D<Real> const & C) noexcept
    : m_Pa(A)
    , m_Pb(B)
    , m_Pc(C)
    {}
    ...
    void setup(Point2D<Real> const & A, 
               Point2D<Real> const & B, 
               Point2D<Real> const & C) noexcept {
      m_Pa = A;
      m_Pb = B;
    ...
    Real signed_area() const noexcept {
      return ((m_Pb.x() - m_Pa.x()) * (m_Pc.y() - m_Pa.y()) -
              (m_Pc.x() - m_Pa.x()) * (m_Pb.y() - m_Pa.y())) * Real(0.5);
    }
    ...
    bool is_valid(Real tol = Real(1e-10)) const noexcept {
      return area() > tol;
    }
    ...
    std::array<Segment2D<Real>, 3> edges() const noexcept {
      return {{
        Segment2D<Real>(m_Pa, m_Pb),
        Segment2D<Real>(m_Pb, m_Pc),
        Segment2D<Real>(m_Pc, m_Pa)
      }};
    }
    ...
    bool contains(Point2D<Real> const & P, Real tol = Real(1e-10)) const {
      ...
      Real const area1 = ((P.x() - m_Pb.x()) * (m_Pc.y() - m_Pb.y()) -
                         (P.y() - m_Pb.y()) * (m_Pc.x() - m_Pb.x())) * Real(0.5);
      Real const area2 = ((P.x() - m_Pc.x()) * (m_Pa.y() - m_Pc.y()) -
                         (P.y() - m_Pc.y()) * (m_Pa.x() - m_Pc.x())) * Real(0.5);
      Real const area3 = ((P.x() - m_Pa.x()) * (m_Pb.y() - m_Pa.y()) -
                         (P.y() - m_Pa.y()) * (m_Pb.x() - m_Pa.x())) * Real(0.5);
    ...
    bool barycentric_coords(Point2D<Real> const & P, 
                           Real & u, Real & v, Real & w,
                           Real tol = Real(1e-10)) const {
      Real const total_area = signed_area();
      if (std::abs(total_area) < tol) return false;
    ...
    Point2D<Real> from_barycentric(Real u, Real v, Real w) const noexcept {
    ...
    bool is_right_angled(Real tol = Real(1e-6)) const {
      ...
      return std::abs(b2 + c2 - a2) < tol || 
             std::abs(a2 + c2 - b2) < tol || 
             std::abs(a2 + b2 - c2) < tol;
    }
    ...
    bool circumcircle(Point2D<Real> & center, Real & radius) const {
      Real const D = 2 * (m_Pa.x() * (m_Pb.y() - m_Pc.y()) +
                         m_Pb.x() * (m_Pc.y() - m_Pa.y()) +
      ...
      center.x((a2 * (m_Pb.y() - m_Pc.y()) +
                b2 * (m_Pc.y() - m_Pa.y()) +
                c2 * (m_Pa.y() - m_Pb.y())) / D);
      center.y((a2 * (m_Pc.x() - m_Pb.x()) +
                b2 * (m_Pa.x() - m_Pc.x()) +
                c2 * (m_Pb.x() - m_Pa.x())) / D);
    ...
    void translate(Real dx, Real dy) noexcept {
      m_Pa.x(m_Pa.x() + dx); m_Pa.y(m_Pa.y() + dy);
      m_Pb.x(m_Pb.x() + dx); m_Pb.y(m_Pb.y() + dy);
      m_Pc.x(m_Pc.x() + dx); m_Pc.y(m_Pc.y() + dy);
    }
  ...
  public:
    ...
    explicit Polygon2D(std::vector<Point2D<Real>> const & points) {
      ...
      for (size_t i = 0; i < points.size(); ++i) {
        set_vertex(static_cast<int>(i), points[i]);
      }
    }
    ...
    Real perimeter() const noexcept {
      ...
      Real p = 0;
      for (int i = 0; i < n; ++i) {
        p += vertex(i).distance(vertex((i + 1) % n));
      }
      return p;
    }
    ...
    bool is_convex(Real tol = Real(1e-10)) const {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        Real cross = (p1.x() - p0.x()) * (p2.y() - p1.y()) -
                    (p1.y() - p0.y()) * (p2.x() - p1.x());
    ...
    bool contains(Point2D<Real> const & P) const {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        if (Vi.y() <= P.y()) {
          if (Vj.y() > P.y()) {
            // Upward crossing
            Real is_left = (Vj.x() - Vi.x()) * (P.y() - Vi.y()) -
                          (P.x() - Vi.x()) * (Vj.y() - Vi.y());
            if (is_left > 0) {
              ++wn;
            }
    ...
    void simplify(Real tol = Real(1e-6)) {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        Real area = std::abs((B.x() - A.x()) * (C.y() - A.y()) -
                            (C.x() - A.x()) * (B.y() - A.y())) * Real(0.5);
