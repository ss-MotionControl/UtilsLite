...
namespace Utils {
  ...
  private:
    ...
    integer   m_block_size;        // Dimensione del blocco (numero di equazioni per iterazione)
  ...
  private:
      ...
      switch (m_strategy) {
        case CYCLIC: {
          ...
          for (integer i = 0; i < actual_block_size; ++i) {
            block_indices.push_back((start + i) % m);
          }
          break;
        }
        ...
        case RANDOM_UNIFORM: {
          ...
          for (integer i = 0; i < actual_block_size; ++i) {
            block_indices.push_back(dist(m_random_engine));
          }
          break;
        }
        ...
        case GREEDY: {
          ...
          for (integer i = 0; i < m; ++i) {
            residuals_with_idx.emplace_back(std::abs(f(i)), i);
          }
          
          // Ordina per residuo decrescente
          std::partial_sort(residuals_with_idx.begin(), 
    ...
    void extract_block(NonlinearSystem& system, const Vector& x, 
                       const Vector& f_full, const std::vector<integer>& block_indices,
                       Matrix& J_block, Vector& f_block) {
      ...
      for (integer i = 0; i < block_size; ++i) {
        ...
        for (SparseMatrix::InnerIterator it(J_full, row_idx); it; ++it) {
          J_block(i, it.col()) = it.value();
        }
      }
    }
    ...
    Vector compute_step(const Matrix& J_block, const Vector& f_block, real_type lambda) {
      ...
      if (solver.info() != Eigen::Success) {
        ...
        Vector inv_singular = singular_values.array() / 
                              (singular_values.array().square() + lambda);
        return -svd.matrixV() * inv_singular.asDiagonal() * 
               svd.matrixU().transpose() * f_block;
      }
    ...
    real_type compute_reduction_ratio(NonlinearSystem& system, const Vector& x,
                                      real_type norm_f_old,
                                      const Vector& dx, const Matrix& J_block,
                                      const Vector& f_block, real_type lambda) {
      ...
      if (std::abs(predicted_reduction) < 1e-12) {
        return (actual_reduction > 0) ? 1.0 : 0.0;
      }
    ...
    real_type armijo_line_search(NonlinearSystem& system, const Vector& x,
                                 real_type norm_f_old,
                                 const Vector& dx, const Matrix& J_block,
    ...
    bool check_convergence(real_type norm_f, real_type initial_norm) const {
      ...
      if (initial_norm > EPSILON && norm_f / initial_norm < m_relative_tolerance) {
        return true;
      }
    ...
    void print_iteration_info(integer iter, real_type norm_f, 
                             real_type initial_norm, real_type lambda,
                             real_type rho, real_type dx_norm) const {
    ...
    void print_summary(real_type initial_norm, real_type final_norm) const {
      ...
      if (m_converged) {
        fmt::print(fmt::fg(fmt::color::green), "Status:             ✓ CONVERGED\n");
      } else {
  ...
  public:
    ...
    bool solve(NonlinearSystem & system, Vector & x) {
      ...
      for (m_num_iterations = 0; 
           m_num_iterations < m_max_iterations && m_num_function_evals < m_max_function_evals; 
           ++m_num_iterations) {
        ...
        if (m_use_line_search) {
          ...
          if (alpha <= 0.0) {
            if (m_verbose_level >= 2) {
              fmt::print(fmt::fg(fmt::color::red), "  ✗ Line search failed\n");
            }
        ...
        real_type rho = compute_reduction_ratio(system, x, norm_f, 
                                                alpha * dx, J_block, f_block, m_lambda);
        
        // Stampa info dettagliate
        print_iteration_info(m_num_iterations, norm_f, m_initial_residual, 
                            m_lambda, rho, dx_norm);
        ...
        if (m_adaptive_lambda) {
          if (rho > m_good_reduction) {
            ...
            if (m_verbose_level >= 3) {
              fmt::print(fmt::fg(fmt::color::green), "  ✓ Good reduction (ρ={:.3f}), decreasing λ: {:.2e} → {:.2e}\n", 
            ...
            if (m_verbose_level >= 2) {
              fmt::print(fmt::fg(fmt::color::red), "  ✗ Bad reduction (ρ={:.3f}), increasing λ: {:.2e} → {:.2e}\n", 
                        rho, old_lambda, m_lambda);
            }
          }
        } else {
      ...
      if (m_verbose_level > 0) {
        if (m_converged) {
          fmt::print(fmt::fg(fmt::color::green), "✓ Finished: {} iterations, {} function evals, final residual: {:.2e}\n", 
                    m_num_iterations, m_num_function_evals, norm_f);
        } else {
          fmt::print(fmt::fg(fmt::color::yellow), "⚠ Finished: {} iterations, {} function evals, final residual: {:.2e}\n", 
                    m_num_iterations, m_num_function_evals, norm_f);
        }
        print_summary(m_initial_residual, norm_f);
      }
