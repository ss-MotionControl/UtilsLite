...
namespace Utils {
  ...
  private:
    ...
    void apply_boundary_constraints(Vector& individual, const Vector& original) {
      ...
      switch (m_constraint_method) {
        case BOUNCE_BACK:
          for (integer j = 0; j < m_dimension; ++j) {
            ...
            if (value < m_lower_bounds[j]) {
              value = m_lower_bounds[j] + m_uniform_dist(m_random_engine) * 
                      (original[j] - m_lower_bounds[j]);
            } else if (value > m_upper_bounds[j]) {
              value = m_upper_bounds[j] + m_uniform_dist(m_random_engine) * 
                      (original[j] - m_upper_bounds[j]);
            }
          }
          break;
        ...
        case CLAMPING:
          for (integer j = 0; j < m_dimension; ++j) {
            individual[j] = std::max(m_lower_bounds[j], 
                                    std::min(m_upper_bounds[j], individual[j]));
          }
          break;
        ...
        case RANDOM_REINIT:
          for (integer j = 0; j < m_dimension; ++j) {
            if (individual[j] < m_lower_bounds[j] || individual[j] > m_upper_bounds[j]) {
              individual[j] = m_lower_bounds[j] + m_uniform_dist(m_random_engine) * 
                             (m_upper_bounds[j] - m_lower_bounds[j]);
            }
          }
          break;
        ...
        case COSINE_TRANSFORM:
          for (integer j = 0; j < m_dimension; ++j) {
            ...
            if (value < m_lower_bounds[j] || value > m_upper_bounds[j]) {
              ...
              real_type transformed = std::asin(2.0 * (value - m_lower_bounds[j]) / 
                                               (m_upper_bounds[j] - m_lower_bounds[j]) - 1.0);
              // Add small random perturbation
              transformed += (m_uniform_dist(m_random_engine) - 0.5) * 0.1;
              // Transform back
              value = m_lower_bounds[j] + (std::sin(transformed) + 1.0) * 0.5 * 
                     (m_upper_bounds[j] - m_lower_bounds[j]);
            }
          }
          break;
    ...
    real_type calculate_diversity() const {
      ...
      for (integer i = 0; i < m_population_size; ++i) {
        diversity += (m_population.col(i) - mean).squaredNorm();
      }
    ...
    void adapt_parameters(real_type F, real_type CR, bool success) {
      ...
      if (!m_successful_F.empty()) {
        ...
        m_memory_CR = std::accumulate(m_successful_CR.begin(),
                                     m_successful_CR.end(), 0.0) / 
                     m_successful_CR.size();
      }
    }
    ...
    void add_to_archive(const Vector& individual) {
      ...
      if (m_current_archive_size < m_archive.cols()) {
    ...
    void local_search(Vector& solution, real_type& fitness, 
                     ObjectiveFunction&& objective, integer max_iterations = 100) {
  ...
  public:
    ...
    void set_dimension(integer dim) { 
      m_dimension = dim;
    }
    ...
    void set_constraint_method(ConstraintMethod method) {
    ...
    void set_value_to_reach(real_type vtr) { 
    ...
    void set_print_interval(integer interval) { 
    ...
    void enable_population_reduction(bool enable = true) {
    ...
    void enable_archive(bool enable = true) {
      ...
      if (enable) {
        m_archive.resize(m_dimension, m_archive_capacity);
      }
    }
    
    void enable_parallel_evaluation(bool enable = true) {
      m_enable_parallel = enable;
    }
    
    void enable_local_search(bool enable = true) {
      m_use_local_search = enable;
    }
    
    void enable_fitness_cache(bool enable = true) {
      m_use_cache = enable;
    }
    
    void set_diversity_threshold(real_type threshold) {
    ...
    template<typename ObjectiveFunction>
    bool minimize(ObjectiveFunction&& objective, Vector& result) {
      // Reset state
      initialize();
      ...
      for (m_current_iteration = 0; m_current_iteration < m_max_iterations; ++m_current_iteration) {
        ...
        for (integer i = 0; i < m_population_size; ++i) {
          if (trial_fitness[i] < m_fitness[i]) {
            ...
            if (m_use_archive) {
              add_to_archive(m_population.col(i));
            }
            ...
            if (trial_fitness[i] < m_best_fitness) {
              ...
              if (m_new_best_callback) {
                m_new_best_callback(m_best_solution, m_best_fitness);
              }
            }
          }
        }
        ...
        if (m_use_local_search && m_current_iteration > m_max_iterations * 0.8) {
          ...
          if (diversity < m_local_search_threshold) {
            local_search(m_best_solution, m_best_fitness, objective, 
                        m_local_search_iterations);
          }
        }
        
        // Print progress
        if (m_verbose && (m_current_iteration % m_print_interval == 0 || 
            m_current_iteration == m_max_iterations - 1)) {
          print_progress();
        }
        
        // Call iteration callback
        if (m_iteration_callback) {
          m_iteration_callback(m_current_iteration, m_best_fitness);
        }
    ...
    static real_type rastrigin_function(const Vector& x) {
      ...
      for (integer i = 0; i < x.size(); ++i) {
        sum += x[i] * x[i] - A * std::cos(2.0 * M_PI * x[i]);
      }
      return sum;
    }
  ...
  private:
    ...
    void initialize() {
      ...
      if (m_use_archive) {
        m_current_archive_size = 0;
      }
      
      if (m_use_cache) {
        m_fitness_cache.clear();
      }
    }
    
    template<typename ObjectiveFunction>
    void initialize_population(ObjectiveFunction&& objective) {
      if (m_use_bounds) {
          ...
          for (integer i = 0; i < m_population_size; ++i) {
            ...
            for (integer j = 0; j < m_dimension; ++j) {
              individual[j] = m_lower_bounds[j] + 
                             m_uniform_dist(m_random_engine) * 
                             (m_upper_bounds[j] - m_lower_bounds[j]);
            }
    ...
    void generate_trial_population(Matrix& trial_population) {
      ...
      if (m_strategy == RAND_1_DITHER) {
        dither_factor = m_dither_min + m_uniform_dist(m_random_engine) * 
                       (m_dither_max - m_dither_min);
      }
      ...
      for (integer i = 0; i < m_population_size; ++i) {
        ...
        switch (m_strategy) {
          ...
          case RAND_TO_BEST_1: // DE/rand-to-best/1
            mutation_vector = ind_r1 + F * (m_best_solution - ind_r1) + 
                                       F * (ind_r2 - ind_r3);
            break;
          ...
          case EITHER_OR: // Either-or algorithm
            if (m_uniform_dist(m_random_engine) < 0.5) {
              ...
              mutation_vector = ind_r1 + 0.5 * (F + 1.0) * 
                                        (ind_r2 + ind_r3 - 2.0 * ind_r1);
            }
            break;
          ...
          case CURRENT_TO_PBEST: // DE/current-to-pbest/1 (JADE)
            {
              ...
              integer pbest_idx = indices[static_cast<integer>(
                m_uniform_dist(m_random_engine) * p)];
              Vector ind_pbest = m_population.col(pbest_idx);
              
              mutation_vector = original_individual + 
                               F * (ind_pbest - original_individual) + 
                               F * (ind_r1 - ind_r2);
            }
            break;
        ...
        if (m_use_bounds) {
          apply_boundary_constraints(mutation_vector, original_individual);
        }
    ...
    template<typename ObjectiveFunction>
    void evaluate_trial_population(Matrix& trial_population, Vector& trial_fitness,
      ...
      if (m_enable_parallel) {
        ...
        for (integer i = 0; i < m_population_size; ++i) {
          ...
          if (m_use_cache) {
            m_fitness_cache[hash_vector(individual)] = trial_fitness[i];
          }
    ...
    bool check_convergence() {
      ...
      if (m_best_history.size() > static_cast<size_t>(m_stagnation_window)) {
        real_type best_recent = *std::min_element(
          m_best_history.end() - m_stagnation_window,
          m_best_history.end()
        );
    ...
    void print_progress() {
      ...
      fmt::print(
        ...
        m_current_iteration, m_best_fitness, m_weight, m_crossover_rate, 
        m_population_size, diversity
      );
      ...
      if (m_verbose && m_current_iteration % (m_print_interval * 5) == 0) {
        ...
        for (integer j = 0; j < std::min(m_dimension, static_cast<integer>(5)); ++j) {
          ...
          if (j < std::min(m_dimension, static_cast<integer>(5)) - 1) {
            fmt::print(", ");
          }
        }
        if (m_dimension > 5) {
