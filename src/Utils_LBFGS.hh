 ...
 * - **LBFGS_BlockCoordinate**: Enhanced block coordinate L-BFGS with overlapping random consecutive blocks
 *
 ...
 * - **Block Coordinate**: Enhanced block coordinate descent with overlapping blocks and coverage tracking
 *
 * ## Usage Example
 *
 * @code{.cpp}
...
namespace Utils
{
  ...
  public:
    ...
    explicit LBFGS(size_t maxCorrections = 10, size_t problem_dimension = 0)
        : m_capacity(maxCorrections), m_dimension(problem_dimension), m_S(problem_dimension, maxCorrections), m_Y(problem_dimension, maxCorrections), m_rho(maxCorrections), m_alpha(maxCorrections), m_q(problem_dimension), m_r(problem_dimension)
    {
     ...
     */
    size_t
    size() const { return m_current_size; }
  ...
  private:
    ...
    bool
    is_on_lower_bound(Scalar x_i, Scalar lower_i) const { return (x_i <= lower_i + m_epsi * (1 + std::abs(lower_i))); }

    bool
    is_on_upper_bound(Scalar x_i, Scalar upper_i) const { return (x_i >= upper_i - m_epsi * (1 + std::abs(upper_i))); }
    ...
    void
    project_inplace(Vector &x) const { x = x.cwiseMax(m_lower).cwiseMin(m_upper); }
    ...
    void
    print_optimization_statistics(const IterationData &data) const
    {
      if (m_options.verbosity_level < 1)
        return;
  ...
  public:
    ...
    void
    set_indent(string const &indent) { m_indent = indent; }

    Vector const &
    solution() const { return m_x; }
      ...
      for (; iteration < m_options.max_iter; ++iteration)
      {
        ...
        if (!is_valid_descent_direction(pg))
        {
          ...
          if (!is_valid_descent_direction(pg))
          {
            ...
            if (m_options.verbosity_level >= 1)
              fmt::print(
                  LBFGS_PrintColors::ERROR, "{}[LBFGS] Cannot find descent direction, stopping\n",
                  m_indent);
            gnorm = projected_gradient_norm(m_x, m_g);
        ...
        if (!step_opt.has_value())
        {
          if (m_options.verbosity_level >= 1)
            fmt::print(
                LBFGS_PrintColors::WARNING, "{}[LBFGS] line search failed, trying fallback steps\n",
                m_indent);
        ...
        {
          Scalar x_change = (m_x_new - m_x).norm();
          if (x_change < m_options.x_tol)
          {
            if (m_options.verbosity_level >= 1)
              fmt::print(
                  LBFGS_PrintColors::SUCCESS, "{}[LBFGS] Converged by x change: {:.2e} < {:.2e}\n",
        ...
        {
          fmt::print(
              LBFGS_PrintColors::WARNING,
              "{}[LBFGS] curvature condition failed (s^T y = {:.2e}), skipping update\n", m_indent,
              sty);
        }
   ...
   * - Statistical overlap between consecutive blocks is approximately overlap_ratio
  ...
  private:
    ...
    print_outer_iteration_header(
        size_t outer_iter, size_t total_cycles,
        vector<size_t> const &block_indices, size_t block_size) const
    {
      ...
      fmt::print(
          ...
          "    ║ Size: {:<5} Indices: {:<41} ║\n"
          "    ╚════════════════════════════════════════════════════════════════╝\n",
          outer_iter, (outer_iter % total_cycles) + 1, total_cycles,
          block_size, format_index_vector_compact<size_t>(block_indices, 7));
    }

    void
    print_outer_statistics(
        size_t outer_iter, size_t total_outer_iters,
    ...
    void
    print_outer_iteration_start(size_t outer_iter, Scalar f, Scalar gnorm) const
    {
      if (m_options.verbosity_level < 1)
        return;
      ...
      if (!m_coordinate_visits.empty())
      {
        size_t visited_count = std::count_if(
            m_coordinate_visits.begin(),
            m_coordinate_visits.end(),
            [](size_t visits)
            { return visits > 0; });
        current_coverage = static_cast<double>(visited_count) / m_total_coordinates * 100.0;
      }
      ...
      fmt::print(
          ...
          "\n"
          "    ╔════════════════════════════════════════════════════════════════╗\n"
          "    ║                    OUTER ITERATION {:3d} START                   ║\n"
          "    ╠════════════════════════════════════════════════════════════════╣\n"
          "    ║ F = {:<12.6e}     ‖pg‖ = {:<12.6e}    Coverage: {:4.1f}%    ║\n"
          "    ╚════════════════════════════════════════════════════════════════╝\n",
          outer_iter, f, gnorm, current_coverage);
    }

    void
    print_outer_iteration_end(size_t outer_iter, Scalar f, Scalar gnorm, Scalar improvement) const
    {
      if (m_options.verbosity_level < 1)
        return;
      ...
      fmt::print(
          ...
          "    ╚════════════════════════════════════════════════════════════════╝\n",
          outer_iter, f, gnorm, improvement);
    }

    void
    print_blocks_summary(size_t outer_iter, const std::vector<std::vector<size_t>> &blocks) const
    {
      if (m_options.verbosity_level < 2)
        return;

      // Calculate coverage and overlap
      std::set<size_t> all_coords;
      for (const auto &block : blocks)
      {
        all_coords.insert(block.begin(), block.end());
      }
      double coverage = static_cast<double>(all_coords.size()) / m_total_coordinates * 100.0;
      double overlap = (1.0 - static_cast<double>(all_coords.size()) /
                                  (blocks.size() * m_options.block_size)) *
                       100.0;
      ...
      fmt::print(
          LBFGS_PrintColors::INFO,
          "    │ Coverage: {:5.1f}% of coordinates    Overlap: {:5.1f}%             │\n"
          "    └────────────────────────────────────────────────────────────────┘\n",
          coverage, overlap);
    }
    ...
    print_block_result_detail(
        size_t outer_iter, size_t block_idx, size_t total_blocks,
        size_t block_size, const std::vector<size_t> &indices,
        Scalar f_before, Scalar f_after, size_t inner_iters,
        size_t inner_evals, bool success) const
    {
      if (m_options.verbosity_level < 2)
        return;

      Scalar improvement = f_before - f_after;
      auto color = success && improvement > 0 ? LBFGS_PrintColors::SUCCESS : success ? LBFGS_PrintColors::WARNING
      ...
      fmt::print(
          ...
          inner_iters, inner_evals,
          f_before, f_after, improvement);
    }

    void
    print_block_header(
        size_t outer_iter, size_t block_idx, size_t total_blocks,
        std::vector<size_t> const &block_indices, size_t block_size) const
    {
      if (m_options.verbosity_level < 2)
        return;
      ...
      fmt::print(
          ...
          "\n"
          "      ┌────────────────────────────────────────────────────────────────┐\n"
          "      │               Processing Block {:>2d}/{:<2d} - Iteration {:3d}           │\n"
          "      ├────────────────────────────────────────────────────────────────┤\n"
          "      │ Size: {:<4} | Indices: {:<40} │\n"
          "      └────────────────────────────────────────────────────────────────┘\n",
          block_idx + 1, total_blocks, outer_iter,
          block_size, format_index_vector_compact<size_t>(block_indices, 5));
    }
    ...
    /**
     * @brief Generate consecutive interval blocks covering [0, n-1] with guaranteed overlap
     *
    ...
    generate_consecutive_interval_blocks(size_t n_total)
    {
      ...
      if (blocks.size() > 1)
      {
        ...
        if (last_block.size() < m_options.min_block_size)
        {
          ...
          // Remove duplicates? The blocks are consecutive, so no duplicates? Actually, they might overlap.
     ...
     */
    void
    initialize_coverage_tracking(size_t n)
    {
     ...
     */
    void
    calculate_coverage_stats(Result &result) const
    {
      if (m_coordinate_visits.empty())
        return;
    ...
    reset_coverage_tracking()
    {
          ...
          m_outer_iteration_count % m_options.coverage_reset_frequency == 0)
      {
        ...
        {
          fmt::print(
              "    [BlockLBFGS] Reset coverage tracking at outer iteration {}\n",
     ...
     */
    void
    update_coverage_counts(std::vector<size_t> const &block_indices)
    {
  ...
  public:
    ...
    void
    clear_bounds() { m_use_bounds = false; }
        ...
        Linesearch const &linesearch = MoreThuenteLineSearch<Scalar>())
    {
      ...
      if (n <= m_options.block_size)
      {
        ...
        {
          fmt::print(
              "    [BlockLBFGS] Problem dimension {} <= block_size {}, using standard LBFGS\n",
      ...
      for (size_t outer_iter = 0; outer_iter < m_options.max_outer_iterations; ++outer_iter)
      {
        ...
        print_outer_iteration_start(outer_iter, f, projected_gradient_norm(x, g));

        // Generate consecutive interval blocks for this iteration
        auto all_blocks = generate_consecutive_interval_blocks(n);
        print_blocks_summary(outer_iter, all_blocks);
        ...
        for (size_t block_idx = 0; block_idx < all_blocks.size(); ++block_idx)
        {
          ...
          print_block_header(outer_iter, block_idx, all_blocks.size(), block_indices, block_indices.size());
          ...
          if (block_gnorm < m_options.lbfgs_g_tol * 0.1)
          {
            ...
            {
              fmt::print(
                  "    [BlockLBFGS] Block gradient too small ({:.2e} < {:.2e}), skipping\n",
          ...
          {
            fmt::print(
                "    [BlockLBFGS] Starting inner L-BFGS on block [{}] (size: {}, ‖g‖∞: {:.2e})\n",
          ...
          bool block_made_progress = (block_improvement > m_options.progress_threshold * std::abs(f_before_block));
          ...
          print_block_result_detail(
              outer_iter, block_idx, all_blocks.size(), block_indices.size(), block_indices,
              f_before_block, f, inner_result.iterations,
              inner_result.function_evaluations, block_made_progress );
        }

        // STAMPA FINE OUTER ITERATION
        Scalar total_improvement = f_start_cycle - f;
        print_outer_iteration_end(outer_iter, f, projected_gradient_norm(x, g), total_improvement);
        ...
        {
          // Good progress - consider increasing block size if we're making progress
        ...
        double current_coverage = 0.0;
        if (m_options.track_coverage && !m_coordinate_visits.empty())
        {
          size_t visited_count = 0;
          for (size_t visits : m_coordinate_visits)
          {
          ...
          current_coverage = static_cast<double>(visited_count) / m_total_coordinates;
          result.coverage_ratio = current_coverage;
        }

        // Print outer statistics periodically
        if (m_options.verbosity_level >= 1 && (outer_iter % 5 == 0 || outer_iter == m_options.max_outer_iterations - 1))
        {
          print_outer_statistics(
              outer_iter, m_options.max_outer_iterations,
        ...
        if (!converged && std::abs(relative_improvement) < m_options.progress_threshold)
        {
          m_stagnation_count++;
          if (m_stagnation_count >= 5)
          {
            // Require more consecutive stagnations
            if (m_options.verbosity_level >= 1)
            {
              fmt::print(
                  "    [BlockLBFGS] Converged by stagnation ({} iterations with Δf < {:.2e})\n",
        ...
        // Check if we've visited all coordinates sufficiently (coverage-based convergence)
        if (!converged && m_options.track_coverage && current_coverage >= m_options.min_coverage_ratio)
        {
          // Only consider coverage-based convergence if we're not making progress
          if (m_stagnation_count >= 3)
          {
            if (m_options.verbosity_level >= 1)
            {
              fmt::print(
                  "    [BlockLBFGS] Converged by coverage ({:.1f}%) after stagnation\n",
        ...
        if (m_options.track_coverage && !check_coverage_adequate())
        {
          ...
          {
            fmt::print(
                "    [BlockLBFGS] Poor coverage detected ({:.1f}% < {:.1f}%), adjusting block selection\n",
                current_coverage * 100, m_options.min_coverage_ratio * 100);
          }
        }
      ...
      if (m_options.verbosity_level >= 1)
      {
        print_outer_statistics(
            result.outer_iterations, m_options.max_outer_iterations,
            total_inner_iters, total_evals, f, true);

        fmt::print(
            "    [BlockLBFGS] Finished: status={}, f_final={:.6e}, total_evals={}\n",
            static_cast<int>(result.status), result.final_function_value, total_evals);
        if (m_options.track_coverage)
        {
          fmt::print(
              "    [BlockLBFGS] Final coverage: {:.1f}% of coordinates visited\n",
