 ...
 */
template <typename T>
std::string format_index_vector_compact(std::vector<T> const &indices,
...
private:
  ...
  size_t m_newest_index{
      0}; ///< Circular buffer index where next pair will be stored
...
public:
  ...
  explicit LBFGS(size_t maxCorrections = 10, size_t problem_dimension = 0)
      : m_capacity(maxCorrections), m_dimension(problem_dimension),
        m_S(problem_dimension, maxCorrections),
        m_Y(problem_dimension, maxCorrections), m_rho(maxCorrections),
        m_alpha(maxCorrections), m_q(problem_dimension),
   ...
   */
  bool add_correction(Vector const &s, Vector const &y,
                      Scalar const min_curvature_ratio = 1e-8) {
    assert(s.size() == y.size());
    if (static_cast<size_t>(s.size()) != m_dimension)
      resize(s.size());
   ...
   *
   * @note If no pairs are stored (size() == 0), returns h0 * g (scaled steepest
   * descent)
   *
  ...
  Scalar compute_initial_h0(Scalar default_value = Scalar(1.0)) const {
    ...
    size_t latest_idx =
        (m_newest_index == 0) ? m_capacity - 1 : m_newest_index - 1;
   ...
   */
  bool add_correction_with_damping(LBFGS<Scalar> &lb, Vector const &s,
                                   Vector const &y,
                                   Scalar const min_curvature_ratio = 1e-8) {
    using Vector = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
    Scalar const sty = s.dot(y);
    ...
    Scalar sBs =
        s.squaredNorm() / h0; // s^T (B0 s) with B0 = H0^{-1} = (1/h0) I
    ...
    if (abs(denom) < std::numeric_limits<Scalar>::epsilon()) {
      ...
      // choose theta so that s^T y_hat = thresh (solve theta*(sty - sBs) + sBs
      // = thresh)
...
public:
  ...
  struct Result {
    ...
    size_t line_search_evaluations{
        0}; // Aggiunto per tracciare eval line search
  };
  ...
  struct Options {
    ...
    size_t verbosity_level{
        1}; // 0: quiet, 1: outer stats, 2: inner progress, 3: detailed
...
private:
  ...
  bool is_on_lower_bound(Scalar x_i, Scalar lower_i) const {
    return (x_i <= lower_i + m_epsi * (1 + std::abs(lower_i)));
  }

  bool is_on_upper_bound(Scalar x_i, Scalar upper_i) const {
    return (x_i >= upper_i - m_epsi * (1 + std::abs(upper_i)));
  }

  void check_bounds_consistency() const {
  ...
  void project_inplace(Vector &x) const {
  ...
  void projected_gradient_inplace(const Vector &x, Vector &g) const {
    ...
    for (int i = 0; i < x.size(); ++i) {
      ...
      // significativo
      if ((on_lower && grad_neg) || (on_upper && grad_pos)) {
        g[i] = Scalar(0);
      }
    }
  }
  void projected_direction_inplace(const Vector &x, Vector &d) const {
    ...
    for (int i = 0; i < x.size(); ++i) {
      ...
      if ((on_lower && dir_neg) || (on_upper && dir_pos)) {
        d[i] = Scalar(0);
      }
    }
  }
  ...
  bool is_valid_descent_direction(Scalar pg) const {
    if (!std::isfinite(pg))
      return false;
    if (pg >= -m_epsi * (1 + std::abs(pg)))
      return false;
    return true;
  }
  ...
  void print_optimization_header(size_t n, Scalar f0) const {
    ...
    fmt::print(
        ...
        "{}"
        "╚════════════════════════════════════════════════════════════════╝\n",
        m_indent, m_indent, m_indent, m_indent,
        fmt::format("Dimension: {:d}", n), m_indent,
        ...
        fmt::format("Bounds: {}",
                    (m_options.use_projection ? "Active" : "None")),
        m_indent);
    fmt::print("{}Initial F = {:.6e}\n", m_indent, f0);
  }

  void print_iteration_summary(size_t iter, Scalar f, Scalar gnorm, Scalar step,
                               Scalar pg, bool improved) const {
    ...
    auto color =
        improved ? LBFGS_PrintColors::SUCCESS : LBFGS_PrintColors::WARNING;
  ...
  void print_line_search_result(size_t iter, Scalar step, size_t evals,
                                bool success) const {
    if (m_options.verbosity_level < 3)
      return;

    auto color =
        success ? LBFGS_PrintColors::SUCCESS : LBFGS_PrintColors::ERROR;
  ...
  void print_lbfgs_update(size_t iter, Scalar sty, bool accepted) const {
    if (m_options.verbosity_level < 3)
      return;

    auto color =
        accepted ? LBFGS_PrintColors::SUCCESS : LBFGS_PrintColors::WARNING;
  ...
  void print_convergence_info(Status status, Scalar gnorm, Scalar f_change,
                              Scalar x_change) const {
    if (m_options.verbosity_level < 1)
      return;
  ...
  void print_optimization_statistics(Result const &data) const {
    if (m_options.verbosity_level < 1)
      return;
    ...
    fmt::print(
        ...
        "{}"
        "╚════════════════════════════════════════════════════════════════╝\n",
        m_indent, m_indent, m_indent, m_indent, status_str, m_indent,
        data.final_function_value, m_indent, data.initial_function_value,
        m_indent, data.total_iterations, m_indent, data.total_evaluations,
  ...
  mutable size_t m_line_search_evaluations{
      0}; // Aggiunto per tracciare eval line search

public:
  LBFGS_minimizer(Options opts = Options())
      : m_options(opts), m_LBFGS(opts.m) {}
  ...
  void reset_memory() {
    if (m_options.verbosity_level >= 2)
      fmt::print("{}[LBFGS] Periodic memory reset\n", m_indent);
    m_LBFGS.clear();
  ...
  minimize(Vector const &x0, Callback const &callback,
    ...
    for (; iteration < m_options.max_iter; ++iteration) {
      ...
      if (m_iter_since_reset >= m_options.iter_reset)
        reset_memory();
      ...
      if (!is_valid_descent_direction(pg)) {
        ...
        if (!is_valid_descent_direction(pg)) {
          ...
          pg = m_p.dot(m_g);
          if (m_options.verbosity_level >= 1)
            fmt::print(LBFGS_PrintColors::ERROR,
                       "{}[LBFGS] Cannot find descent direction, stopping\n",
                       m_indent);
          gnorm = projected_gradient_norm(m_x, m_g);
      ...
      if (!step_opt.has_value()) {
        if (m_options.verbosity_level >= 1)
          fmt::print(LBFGS_PrintColors::WARNING,
                     "{}[LBFGS] line search failed, trying fallback steps\n",
                     m_indent);
      ...
      Scalar sty_tolerance =
          std::max(m_options.sty_min_factor,
                   m_epsi * m_s.squaredNorm() * m_y.squaredNorm());

      bool update_accepted = false;
      if (sty > sty_tolerance) {
        update_accepted = m_LBFGS.add_correction(m_s, m_y);
      }

      print_lbfgs_update(iteration, sty, update_accepted);

      if (!update_accepted && m_options.verbosity_level >= 2 &&
          m_LBFGS.size() > 0) {
        fmt::print(LBFGS_PrintColors::WARNING,
      ...
      if (f_change <= m_options.f_tol) {
        ...
        if (gnorm <= m_options.g_tol_weak) {
          if (m_options.verbosity_level >= 1)
            fmt::print(
                LBFGS_PrintColors::SUCCESS,
                "{}[LBFGS] Converged by function change: {:.2e} < {:.2e}\n",
                m_indent, f_change, m_options.f_tol);
      ...
      // Check for stagnation in variables
      {
        ...
        if ( x_change < m_options.x_tol ) {
          ...
          if (gnorm <= m_options.g_tol_weak) {
            if (m_options.verbosity_level >= 1)
              fmt::print(LBFGS_PrintColors::SUCCESS,
  ...
  exit_position:
    Result result{status, iteration, m_function_evaluations,
                  gnorm,  f,         f_initial};
...
public:
  ...
  struct Options {
    ...
    Scalar absolute_progress_threshold{
        1e-12}; // Absolute threshold for tiny improvements
...
private:
  ...
  void print_outer_statistics(size_t total_outer_iters,
    ...
    fmt::print(
        ...
        m_outer_iteration_count, total_inner_iters, total_evals, best_value,
        (converged ? "CONVERGED" : "RUNNING"));
  }

  void print_block_coordinate_header(size_t n, Scalar f0, Scalar g0) const {
    if (m_options.verbosity_level < 1)
      return;
  ...
  void print_outer_iteration_end(Scalar f, Scalar gnorm,
                                 Scalar improvement) const {
    if (m_options.verbosity_level < 1)
      return;
  ...
  print_blocks_summary(std::vector<std::vector<size_t>> const &blocks) const {
    ...
    for (size_t i = 0; i < blocks.size(); ++i) {
      fmt::print(LBFGS_PrintColors::INFO,
  ...
  void print_block_result_detail(size_t block_idx, size_t total_blocks,
                                 Scalar f_before, Scalar f_after,
                                 size_t inner_iters, size_t inner_evals,
                                 bool success) const {
    if (m_options.verbosity_level < 2)
      return;

    Scalar improvement = f_before - f_after;
    auto color = success && improvement > 0 ? LBFGS_PrintColors::SUCCESS
                 : success                  ? LBFGS_PrintColors::WARNING
                                            : LBFGS_PrintColors::ERROR;

    string status_icon = success ? (improvement > 0 ? "✅" : "⚡") : "❌";
    string status_text =
        success ? (improvement > 0 ? "IMPROVED" : "NO CHANGE") : "FAILED";
    ...
    fmt::print(
        ...
        "      "
        "└────────────────────────────────────────────────────────────────┘\n",
        block_idx + 1, total_blocks, status_icon, status_text, inner_iters,
        inner_evals, f_before, f_after, improvement);
  }

  void print_block_header(size_t block_idx, size_t total_blocks,
  ...
  generate_consecutive_interval_blocks(size_t n_total) {
    ...
    size_t block_size =
        std::clamp(m_options.block_size, m_options.min_block_size,
                   std::min(m_options.max_block_size, n_total));

    // Calculate step size based on overlap ratio
    size_t step =
        static_cast<size_t>(block_size * (1.0 - m_options.overlap_ratio));
    ...
    if (blocks.size() > 1) {
      ...
      if (last_block.size() < m_options.min_block_size) {
        ...
        prev_block.insert(prev_block.end(), last_block.begin(),
                          last_block.end());
        ...
        prev_block.erase(std::unique(prev_block.begin(), prev_block.end()),
                         prev_block.end());
  ...
  void adapt_block_size(Scalar improvement, size_t n_total) {
    if (!m_options.adaptive_block_size)
      return;

    Scalar relative_improvement =
        std::abs(improvement) / (1.0 + std::abs(m_previous_best));

    // Adjust based on progress
    if (relative_improvement > 0.1) {
      // Good progress - increase block size
      m_options.block_size =
          std::min(m_options.max_block_size, m_options.block_size + 5);
    } else if (relative_improvement < 0.001) {
      // Poor progress - decrease block size
      m_options.block_size =
          std::max(m_options.min_block_size, m_options.block_size - 2);
    }
  ...
  Vector extract_subvector(Vector const &full,
  ...
  void update_full_vector(Vector &full, Vector const &sub,
                          std::vector<size_t> const &indices) {
    for (size_t i = 0; i < indices.size(); ++i) {
  ...
  void extract_subbounds(std::vector<size_t> const &indices, Vector &sub_lower,
                         Vector &sub_upper) {
...
public:
  ...
  template <typename Linesearch>
  Result
  minimize(Vector const &x0, Callback const &global_callback,
           Linesearch const &linesearch = MoreThuenteLineSearch<Scalar>()) {
    Result result;
    size_t n = x0.size();
    ...
    if (n <= m_options.block_size) {
      if (m_options.verbosity_level >= 1) {
        fmt::print("    [BlockLBFGS] Problem dimension {} <= block_size {}, "
      ...
      opts.max_iter =
          m_options.max_outer_iterations * m_options.max_inner_iterations;
      ...
      result.status = (standard_result.status ==
                           LBFGS_minimizer<Scalar>::Status::CONVERGED ||
                       standard_result.status ==
                           LBFGS_minimizer<Scalar>::Status::GRADIENT_TOO_SMALL)
                          ? Status::CONVERGED
                          : Status::FAILED;
    ...
    while (m_outer_iteration_count < m_options.max_outer_iterations) {
      ...
      for (size_t block_idx = 0; block_idx < all_blocks.size(); ++block_idx) {
        ...
        print_block_header(block_idx, all_blocks.size(), block_indices,
                           block_indices.size());
        ...
        if (block_gnorm < m_options.lbfgs_g_tol * 0.1) {
          if (m_options.verbosity_level >= 2) {
            fmt::print("    [BlockLBFGS] Block gradient too small ({:.2e} < "
                       "{:.2e}), skipping\n",
        ...
        auto block_callback = [&](Vector const &x_sub,
                                  Vector *g_sub) -> Scalar {
          Vector x_full = m_x;
        ...
        Scalar adaptive_f_tol =
            std::max(m_options.lbfgs_f_tol,
                     m_options.absolute_progress_threshold / f_scale);
        ...
        max_block_improvement =
            std::max(max_block_improvement, block_improvement);

        // Enhanced acceptance criteria: both relative AND absolute
        Scalar relative_improvement =
            block_improvement / (std::abs(f_before_block) + Scalar(1e-16));
        Scalar absolute_improvement = block_improvement;

        bool block_made_progress =
            (relative_improvement > m_options.progress_threshold) ||
        ...
        if (block_made_progress) {
          ...
          best_improvement_in_cycle =
              std::max(best_improvement_in_cycle, block_improvement);
        }

        // STAMPA RISULTATO DETTAGLIATO BLOCCO
        print_block_result_detail(block_idx, all_blocks.size(), f_before_block,
                                  f, inner_result.total_iterations,
      ...
      print_outer_iteration_end(f, projected_gradient_norm(m_x, g),
                                total_improvement);
      ...
      if (made_progress_in_cycle) {
        ...
        if (best_improvement_in_cycle > 0.01 * std::abs(f_start_cycle)) {
          m_options.block_size =
              std::min(m_options.max_block_size, m_options.block_size + 2);
        }
      } else {
        // Poor progress - decrease block size more cautiously
        m_options.block_size =
            std::max(m_options.min_block_size, m_options.block_size - 1);
      }
      ...
      if (m_options.verbosity_level >= 1 &&
          (m_outer_iteration_count % 5 == 0 ||
           m_outer_iteration_count == m_options.max_outer_iterations - 1)) {
        print_outer_statistics(m_options.max_outer_iterations,
      ...
      if (gnorm < m_options.outer_tolerance) {
        if (m_options.verbosity_level >= 1) {
          fmt::print(
              "    [BlockLBFGS] Converged by gradient norm: {:.2e} < {:.2e}\n",
      ...
      Scalar relative_stagnation =
          cycle_improvement / (std::abs(f_start_cycle) + Scalar(1e-16));

      // Scalar relative_stagnation_tol{1e-8};  // Tighter relative tolerance

      // Check function value stagnation
      Scalar relative_improvement =
          f_improvement / (1.0 + std::abs(f_start_cycle));
      if (!converged && relative_improvement < m_options.progress_threshold) {
        ...
        if (m_stagnation_count >= m_options.max_stagnation_count) {
          ...
          if (m_options.verbosity_level >= 1) {
            fmt::print("    [BlockLBFGS] Converged by stagnation ({} "
    ...
    if (m_options.verbosity_level >= 1) {
      print_outer_statistics(m_options.max_outer_iterations, total_inner_iters,
                             total_evals, f, true);

      fmt::print("    [BlockLBFGS] Finished: status={}, f_final={:.6e}, "
                 "total_evals={}\n",
                 static_cast<int>(result.status), result.final_function_value,
                 total_evals);
    }
