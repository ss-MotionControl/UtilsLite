...
namespace Utils {
  ...
  public:
    ...
    add_correction( Vector const & s, Vector const & y, Scalar const min_curvature_ratio = 1e-8 ) {
      ...
      // basic curvature check: s'y must be positive and not numerically zero
      if ( !(sty > 0) ) return false;

      Scalar denom { snrm * ynrm };
      if ( denom < 1 ) denom = 1;
      if ( sty < denom * min_curvature_ratio ) return false;
  ...
  namespace detail {
    ...
    inline
    Scalar
    cubic_minimizer( Scalar a, Scalar fa, Scalar fpa, Scalar b, Scalar fb, Scalar fpb, Scalar perc = 0.01 ) {
      ...
      Scalar sqrt_disc { std::sqrt(std::max<Scalar>(Scalar(0), discr)) };
      ...
      // clamp to [a,b]
      Scalar mi{ std::min(a,b) };
      Scalar ma{ std::max(a,b) };
      Scalar d{ perc*(ma-mi) };
      mi += d;
      ma -= d;
      if      ( t < mi ) t = mi;
    ...
    inline
    Scalar
    quadratic_minimizer( Scalar a, Scalar fa, Scalar fpa, Scalar b, Scalar fb, Scalar perc = 0.01 ) {
      // Quadratic minimizer that uses derivative at a and function values at a and b.
      // Fit quadratic q(alpha) = A*(alpha-a)^2 + B*(alpha-a) + C with q(a)=fa, q'(a)=fpa, q(b)=fb.
      Scalar const denom { ( (fb - fa) - fpa*(b - a) ) };
      if ( abs(denom) < std::numeric_limits<Scalar>::epsilon() ) return (a + b) / Scalar(2);
      Scalar alpha { a - (fpa * (b - a) * (b - a)) / (2 * denom) };
      // clamp to [a,b]
      Scalar mi{ std::min(a,b) };
      Scalar ma{ std::max(a,b) };
      Scalar d{ perc*(ma-mi) };
      mi += d;
      ma -= d;
      if      ( alpha < mi ) alpha = mi;
      else if ( alpha > ma ) alpha = ma;
      return alpha;
    }
  ...
  public:
    ...
    operator()(
      ...
      while ( evals < m_max_iters ) {
        ...
        Scalar new_alpha = std::min( 2 * alpha, m_alpha_max);
        alpha_prev = alpha;
        phi_prev   = phi;
        der_prev   = der;
        alpha      = new_alpha;
      ...
      // --- Zoom phase ---
      while ( evals < m_max_iters ) {
        Scalar a_lo = alpha_lo;
        Scalar a_hi = alpha_hi;
        Scalar a_j;
        if ( abs(a_hi - a_lo) > m_epsi )
          a_j = detail::cubic_minimizer( a_lo, phi_lo, der_lo, a_hi, phi_hi, der_hi );
        else
          a_j = (a_lo + a_hi) / 2;
  ...
  public:
    ...
    operator()(
      ...
      for (size_t k=0; k<m_max_iters; ++k) {
        ...
        Scalar f_new = callback(m_x_new, nullptr);
        if (f_new <= f0 + m_c1*alpha*Df0 && f_new >= f0 + (1-m_c1)*alpha*Df0)
          return alpha;
        alpha *= m_step_reduce;
      }
      return std::nullopt;
  ...
  class HagerZhangLineSearch {
    ...
    zoom(
      ...
      for ( size_t iter{0}; iter < m_max_iters && m_evals < m_max_iters; ++iter ) {
        ...
        Scalar a_j = detail::cubic_minimizer( a_lo, phi_lo, der_lo, a_hi, phi_hi, der_hi );
        
        // Safeguard
        if ( a_j <= std::min(a_lo, a_hi) || a_j >= std::max(a_lo, a_hi) ) a_j = (a_lo + a_hi) / 2;
  ...
  public:
    ...
    operator()(
      ...
      for ( size_t iter = 0; iter < m_max_iters; ++iter ) {
        ...
        // Extrapolation
        Scalar alpha_new = std::min( 2 * alpha, m_alpha_max );
  ...
  public:
    ...
    operator()(
      ...
      for ( size_t k{0}; k < m_max_iters && evals < m_max_iters; ++k ) {
        ...
        // --- Salvaguardia finale ---
        if ( alpha_new <= 0 || alpha_new >= m_alpha_max ) alpha_new = std::min(2.0 * alpha_curr, m_alpha_max);
   ...
   * \brief High-level L-BFGS-based minimizer with optional box constraints.
   *
   * Template on Scalar (e.g., double).
   *
   * Description:
   *   - Takes an objective function callback `f_and_grad(x, &f, &g)` that evaluates
   *     the function value and gradient at x.
   *   - Uses LBFGS two-loop recursion to compute search direction p = - H g.
   *   - Uses Armijo backtracking line-search (strong Wolfe optional to implement).
   *   - If box constraints are provided (lower/upper arrays), it projects iterates
   *     onto the box and uses a projected direction (zeroing components at bounds
   *     where gradient pushes outside).
   *
   * Note on LBFGS-B:
   *   - The original LBFGS-B uses a more sophisticated active-set approach to
   *     solve the box-constrained QP subproblem; here we provide a practical,
   *     well-performing projected L-BFGS variant suitable for many applications.
   */
  template <typename Scalar = double>
  class LBFGS_minimizer2 {
  public:
    using Vector   = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>;
    ...
    struct Options {
      ...
      Scalar g_tol           { 1e-8  }; ///< gradient norm tolerance
      Scalar f_tol           { 1e-12 }; ///< function change tolerance
      Scalar step_max        { 1     }; ///< initial step
      Scalar c1              { 1e-4  }; ///< Armijo constant
      Scalar step_reduce     { 0.5   };
      size_t max_line_search { 40    };
      size_t m               { 10    }; ///< LBFGS memory
      bool   use_projection  { false }; ///< enable box constraints projection
      bool   verbose         { false };
    };
  ...
  public:
     ...
     * \brief Constructor
     * \param n dimension
     * \param opts options structure (see Options)
     */
    LBFGS_minimizer2( Options opts = Options() )
    : m_options(opts)
    , m_LBFGS(opts.m)
    {}

    /// Set box bounds (use +/- inf to disable)
    void
    ...
    /// Clear stored LBFGS memory
    void reset_memory() { m_LBFGS.clear(); }

    /*!
     * \brief Minimize starting from x0. The callback must fill f and g:
     *
     *   f = callback(x, &g) : evaluates f(x) and gradient g
     *
     * Returns pair (status, final_x). status: 0=converged, 1=maxiter, 2=line_search_failed
     */
    ...
    minimize(
      Vector     const & x0,
      Callback   const & callback,
      Linesearch const & linesearch = ArmijoLineSearch<Scalar>()
    ) {
      ...
      // project initial x in bounds
      if ( m_options.use_projection ) project_inplace(m_x);

      Scalar f_prev { f };
      for ( size_t iter{ 0 }; iter < m_options.max_iter; ++iter ) {
        // Check gradient norm (projected gradient if bounds)
        Scalar gnorm{ projected_gradient_norm(m_x, m_g) };
        if ( m_options.verbose ) fmt::printf( "[LBFGS] iter=%-3d f=%-12.4g ‖pg‖∞=%-12.4g\n", iter, f, gnorm );
        ...
        // If projection enabled, zero components where variable is at bound and gradient pushes outside
        if ( m_options.use_projection ) {
          projected_direction_inplace( m_x, m_p );
          //auto freeze = ((m_x.array() <= (m_lower.array() + m_epsi)) && (m_p.array() < 0)) ||
          //              ((m_x.array() >= (m_upper.array() - m_epsi)) && (m_p.array() > 0));
          //m_p = freeze.select(Vector::Zero(n), m_p);
          if ( m_p.isZero() ) m_p = -projected_gradient(m_x, m_g);
        }

        // Ensure descent direction; if not, use negative gradient
        ...
        if ( !(pg < 0) ) {
          ...
          if ( !(pg < 0) ) {
            // give up (no descent), declare done
            if (m_options.verbose) fmt::print( "[LBFGS] No descent direction, stopping.\n" );
            return {0, m_x, f};
          }
        }

        // ---- LINE SEARCH POLICY CALL ---------------------------------------
        auto step_opt = linesearch( f, pg, m_x, m_p, callback, m_options.step_max );
        
        if (!step_opt.has_value()) {
          if (m_options.verbose)
            fmt::print("[LBFGS] line search failed\n");
          return {2, m_x, f};
        }

        Scalar step = *step_opt;
        ...
        // add correction if acceptable
        bool added{ m_LBFGS.add_correction(m_s,m_y) };
        if (!added) {
          // If not added (curvature issue), optionally clear memory to recover
          if (m_options.verbose) fmt::print( "[LBFGS] correction rejected; clearing memory\n" );
          m_LBFGS.clear();
        }
        ...
        if ( abs(f - f_prev) <= step * m_options.f_tol ) return {0, m_x, f};
        f_prev = f;
      }
  ...
  public:
    ...
    minimize(
      ...
      for ( size_t iter{ 0 }; iter < m_options.max_iter; ++iter ) {
        ...
        if (iter > 0 && x_change < m_options.x_tol) {
          if (m_options.verbose)
        ...
        Scalar sty_tolerance = std::max(m_options.sty_min_factor,
                                      1e-12 * m_s.squaredNorm() * m_y.squaredNorm());
