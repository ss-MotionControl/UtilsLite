...
public:
  ...
  jacobian( Vector const & x, SparseMatrix & J ) const override {
    ...
    // ∂²f/∂x0²
    J_full(0,0) = 100000/x0_cu + tau * (
                  2 * (4/(x0_sq)) * (4/(x0_sq)) +          // from (dP1/dx0)^2
    ...
    // ∂²f/∂x0∂x1
    J_full(0,1) = tau * (
                  2 * (4/(x0_sq)) * (32/(x1_sq)) +         // from (dP1/dx0)(dP1/dx1)
    ...
    // ∂²f/∂x0∂x2
    J_full(0,2) = tau * (
                  2 * (4/(x0_sq)) * (120/(x2_sq)) +        // from (dP1/dx0)(dP1/dx2)
    ...
    // ∂²f/∂x0∂x3
    J_full(0,3) = tau * (
                  2 * (4/(x0_sq)) * (-2*x3) +              // from (dP1/dx0)(dP1/dx3)
                  2 * P1 * 0                               // no cross term
                );
    ...
    // ∂²f/∂x0∂x4
    J_full(0,4) = tau * (
                  2 * 1 * (-2*x4)                          // from (dP2/dx0)(dP2/dx4)
                );
    J_full(4,0) = J_full(0,4);  // Symmetry
    
    // ∂²f/∂x1²
    J_full(1,1) = 144000/x1_cu + tau * (
                  2 * (32/(x1_sq)) * (32/(x1_sq)) +        // from (dP1/dx1)^2
    ...
    // ∂²f/∂x1∂x2
    J_full(1,2) = tau * (
                  2 * (32/(x1_sq)) * (120/(x2_sq)) +       // from (dP1/dx1)(dP1/dx2)
                  2 * P1 * 0                               // no cross term
                );
    ...
    // ∂²f/∂x1∂x3
    J_full(1,3) = tau * (
                  2 * (32/(x1_sq)) * (-2*x3) +             // from (dP1/dx1)(dP1/dx3)
                  2 * P1 * 0                               // no cross term
                );
    ...
    // ∂²f/∂x1∂x5
    J_full(1,5) = tau * (
                  2 * 1 * (-2*x5)                          // from (dP3/dx1)(dP3/dx5)
                );
    J_full(5,1) = J_full(1,5);  // Symmetry
    
    // ∂²f/∂x2²
    J_full(2,2) = 288000/x2_cu + tau * (
                  2 * (120/(x2_sq)) * (120/(x2_sq)) +      // from (dP1/dx2)^2
    ...
    // ∂²f/∂x2∂x3
    J_full(2,3) = tau * (
                  2 * (120/(x2_sq)) * (-2*x3) +            // from (dP1/dx2)(dP1/dx3)
                  2 * P1 * 0                               // no cross term
                );
    ...
    // ∂²f/∂x2∂x6
    J_full(2,6) = tau * (
                  2 * 1 * (-2*x6)                          // from (dP4/dx2)(dP4/dx6)
                );
    J_full(6,2) = J_full(2,6);  // Symmetry
    
    // ∂²f/∂x3²
    J_full(3,3) = tau * (
                  2 * (-2*x3) * (-2*x3) +                  // from (dP1/dx3)^2
                  2 * P1 * (-2)                            // from d²P1/dx3²
                );
    
    // ∂²f/∂x4²
    J_full(4,4) = tau * (
                  2 * (-2*x4) * (-2*x4) +                  // from (dP2/dx4)^2
                  2 * P2 * (-2)                            // from d²P2/dx4²
                );
    
    // ∂²f/∂x5²
    J_full(5,5) = tau * (
                  2 * (-2*x5) * (-2*x5) +                  // from (dP3/dx5)^2
                  2 * P3 * (-2)                            // from d²P3/dx5²
                );
    
    // ∂²f/∂x6²
    J_full(6,6) = tau * (
                  2 * (-2*x6) * (-2*x6) +                  // from (dP4/dx6)^2
  ...
  exact_solution( vector<Vector> & x_vec ) const override {
    ...
    x0.resize(n);
    if ( tau == 1e2 ) {
      x0 << 6.681814179997021692636660004601691302401,
            9.074510495626874674434074592710482202822,
            12.99535539607610906540018013075666371709,
            0, 0, 0, 0;
    } else if ( tau == 1e4 ) {
      x0 << 3.900157915419476405557407980428199299310,
            7.709248543842730875537744129799143332094,
            11.96290763014932833437155935276448655442,
            0, 0, 0, 0;
    } else if ( tau == 1e6 ) {
      x0 << 3.847831075410325147818365927952842675327,
            7.694733977443614783484512361392494920171,
            11.95451849301910537037961219270342443906,
            0, 0, 0, 0;
    } else if ( tau == 1e8 ) {
      x0 << 3.847300847063338217370019344657353290973,
            7.694589110936489763239095186897873067137,
            11.95443542298989582053077640148919021918,
            0, 0, 0, 0;
    } else if ( tau == 1e10 ) {
      x0 << 3.847295544068686165133049,
            7.694587662301036484281052,
            11.95443459237430425571666,
            0, 0, 0, 0;
    } else {
      x_vec.clear();
    }
