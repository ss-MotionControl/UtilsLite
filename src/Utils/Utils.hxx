...
inline string get_filename( string_view path )
{
    namespace fs = std::filesystem;
    return fs::path( path ).filename().string();
}

inline string get_extension( string_view path )
{
    namespace fs = std::filesystem;
...
extern template void search_interval( int32_t npts, double const X[], double& x,
    int32_t& last_interval, bool closed, bool can_extend );

extern template void search_interval(
    int64_t npts, float const X[], float& x, int64_t& last_interval, bool closed, bool can_extend );

extern template void search_interval( int64_t npts, double const X[], double& x,
    int64_t& last_interval, bool closed, bool can_extend );
#endif

template<typename T_int, typename T_real>
inline void searchInterval(
    T_int npts, T_real const X[], T_real& x, T_int& last_interval, bool closed, bool can_extend )
{
    search_interval( npts, X, x, last_interval, closed, can_extend );
}

static inline void to_upper( string& str )
{
    std::transform( str.begin(), str.end(), str.begin(), []( unsigned char c ) -> unsigned char {
        return static_cast<unsigned char>( std::toupper( c ) );
    } );
}
static inline void to_lower( string& str )
{
    std::transform( str.begin(), str.end(), str.begin(), []( unsigned char c ) -> unsigned char {
        return static_cast<unsigned char>( std::tolower( c ) );
    } );
}
static inline bool is_lower( string_view s )
{
    return std::all_of( s.begin(), s.end(), islower );
}
static inline bool is_upper( string_view s )
{
    return std::all_of( s.begin(), s.end(), isupper );
}
static inline bool is_alpha( string_view s )
{
    return std::all_of( s.begin(), s.end(), isalpha );
}
static inline bool is_alphanum( string_view s )
{
    return std::all_of( s.begin(), s.end(), isalnum );
}
static inline bool is_digits( string_view s )
{
    return std::all_of( s.begin(), s.end(), isdigit );
}
static inline bool is_xdigits( string_view s )
{
...
static inline unsigned iLog2( uint64_t N )
{
    static unsigned const tab64[64] = { 63, 0, 58, 1, 59, 47, 53, 2, 60, 39, 48, 27, 54, 33, 42, 3,
        61, 51, 37, 40, 49, 18, 28, 20, 55, 30, 34, 11, 43, 14, 22, 4, 62, 57, 46, 52, 38, 26, 32,
        41, 50, 36, 17, 19, 29, 10, 13, 21, 56, 45, 25, 31, 35, 16, 9, 12, 44, 24, 15, 8, 23, 7, 6,
        5 };
    N |= N >> 1;
    N |= N >> 2;
    N |= N >> 4;
