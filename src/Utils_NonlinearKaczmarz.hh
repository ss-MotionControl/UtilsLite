...
namespace Utils {
  ...
  public:
    ...
    void set_relaxation(real_type lambda) {
      UTILS_ASSERT(
        lambda > 0 && lambda <= MAX_RELAXATION,
        "Relaxation parameter must be in (0, 2]"
      );
      m_relaxation = lambda;
    }
  ...
  private:
    ...
    void compute_gradient(NonlinearSystem& system, const Vector& x, 
                         integer eq_idx, Vector& grad) {
      integer n = system.num_equations();
      
      // Alloca Jacobiano solo se necessario
      if (m_grad_cache.size() != n) {
        m_grad_cache.resize(n);
      }
    ...
    real_type armijo_line_search(NonlinearSystem& system, const Vector& x,
                                 const Vector& f, real_type norm_f,
                                 integer eq_idx, const Vector& grad,
  ...
  public:
    ...
    bool solve(NonlinearSystem& system, Vector& x) {
      ...
      for (m_num_iterations = 0; 
           m_num_iterations < m_max_iterations && 
           m_num_function_evals < m_max_function_evals; 
           ++m_num_iterations) {
        ...
        if (m_verbose_level == 2) {
          ...
          fmt::print(fmt::fg(fmt::color::light_blue), "[{:5}] ‖f‖ = {:.2e}", fmt::format("{}", m_num_iterations + 1), norm_f);
        ...
        if (m_num_iterations > 100 && m_num_iterations % 100 == 0) {
          real_type avg_reduction = std::pow(norm_f / initial_norm, 
                                            1.0 / m_num_iterations);
    ...
    bool solve_block(NonlinearSystem& system, Vector& x, integer block_size = 5) {
      ...
      UTILS_ASSERT(n > 0, "System must have at least one equation");
      UTILS_ASSERT(x.size() == n, "Initial guess size mismatch");
      UTILS_ASSERT(block_size > 0 && block_size <= n, 
                   "Block size must be in [1, n]");
      ...
      for (m_num_iterations = 0; m_num_iterations < m_max_iterations; 
           ++m_num_iterations) {
  ...
  private:
    ...
    std::vector<integer> select_block(const Vector& f, integer n, 
      ...
      switch (m_strategy) {
        ...
        case CYCLIC: {
          ...
          for (integer i = 0; i < block_size; ++i) {
            block_indices.push_back((start + i) % n);
          }
          break;
        }
        ...
        case RANDOM_WEIGHTED: {
          ...
          std::partial_sort(sorted_indices.begin(), 
                          ...
                          sorted_indices.end(),
                          [this](integer i, integer j) {
                            return m_residuals[i] > m_residuals[j];
                          });
          
          for (integer i = 0; i < block_size; ++i) {
            block_indices.push_back(sorted_indices[i]);
          }
          break;
        }
      }
