 ...
 * @brief Defines the ParetoFront class for maintaining the Pareto set in multi-objective optimization.
 */
...
namespace Utils {
  ...
  /**
   * @brief A data structure for maintaining the Pareto Front of a set of N-dimensional points.
   *
   * The **Pareto Front** (or Pareto Set) is the set of non-dominated solutions in a
   * multi-objective optimization problem. A solution (point) $A$ is considered
   * **non-dominated** if no other solution $B$ in the set is superior (better or equal)
   * in all objectives and strictly superior in at least one objective.
   *
   ...
   * and a periodic rebuild strategy (`maybe_rebuild`) for consistent performance
   * during incremental erasures.
   *
   * @tparam T The numeric type for the point coordinates (e.g., int, float, double).
   * @tparam N The dimensionality of the space (number of objectives). Must be > 0.
   * @tparam Payload The type of auxiliary data stored with each point. Defaults to void (std::monostate).
   * @tparam RebuildThreshold The number of deleted entries (`m_tombstones`) that triggers a physical cleanup of the underlying vector.
   *
   * @section concept Dominance Concept
   * The core of the Pareto Front is the dominance relation. Given two points $A$ and $B$:
   * - **A strictly dominates B** if $A$ is non-worse than $B$ in all objectives and strictly better in at least one.
   * - **A weakly dominates B** if $A$ is non-worse than $B$ in all objectives.
   *
   * @section insertion Incremental Insertion (`insert`)
   * The `insert` method guarantees that the front remains correct after adding a new point $P$:
   * 1. Check if $P$ is an exact duplicate (fail).
   * 2. Check if $P$ is weakly dominated by the existing front (fail, $P$ is inferior).
   * 3. **Prune**: Identify and soft-delete all points $E$ in the current front where $P$ strictly dominates $E$.
   * 4. Add $P$ as a new active entry.
   *
   * The use of soft-deletion (`Entry::alive = false`) maintains pointer stability (if entries were accessed by index) but requires periodic cleanup via `maybe_rebuild`.
   *
   * @section batch Batch Construction (`batch_build`)
   * The batch building method is typically more efficient for large, unsorted datasets.
   * It relies on sorting the input based on a scalar measure (L1 norm) to quickly filter out many dominated points.
   * 1. **Sort**: Input points are sorted by their L1 norm (sum of absolute values) plus a lexicographical tie-breaker. This heuristic places promising, low-norm (often near the origin) points earlier.
   * 2. **Iterate & Prune**: Iterate through the sorted points. For each candidate $C$:
   * a. Check if $C$ is weakly dominated by the current *partial* front (expensive).
   * b. If not dominated, remove any point in the partial front that is strictly dominated by $C$.
   * c. Add $C$ to the partial front.
   */
  template <
    typename T,
    size_t   N,
    typename Payload          = void,
    size_t   RebuildThreshold = 128
  >
  class ParetoFront {
  public:
    /// @brief Type alias for an N-dimensional point. $\mathbf{p} \in \mathbb{R}^N$.
    using Point_type   = std::array<T,N>; // Eigen::Matrix<T,N,1>; // ;
    /// @brief Type alias for the payload, resolved to std::monostate if Payload is void.
    using Payload_type = std::conditional_t<std::is_same<Payload,void>::value, std::monostate, Payload>;

    /**
     * @brief Structure representing an entry (a point and its metadata) in the front.
     */
    struct Entry {
      ...
      size_t       id{0};       ///< A unique identifier. Used for reliable deletion/payload retrieval.
    };

    /// @brief Type alias for a vector of point-payload pairs, representing the current front.
    using Front = std::vector<std::pair<Point_type,Payload_type>>;

  private:
    /// @brief Internal storage vector containing all entries (active and tombstones).
    std::vector<Entry>  m_entries;
    /// @brief Counter for soft-deleted entries. When this reaches `RebuildThreshold`, a physical clean-up occurs.
     ...
     * 2. $\exists j \in [0, N-1]$, $A$ is strictly better than $B$ in objective $j$.
     *
     ...
     * removing the tombstones to maintain spatial locality and iteration performance.
     *
     * \dot
     ...
     * 1. **Primary Key**: L1 Norm $\sum_{i=0}^{N-1} |p_i|$. Sorting by ascending norm ensures that points
     * closer to the origin (often better candidates in normalized spaces) are processed first.
  ...
  public:
     ...
     * @param minimize An array of booleans defining the optimization direction for each dimension.
     */
     ...
     * @param f The function to apply to each alive Entry. Signature must accept a const Entry&.
     */
    template <typename F>
    void
    ...
    /**
     * @brief Returns a vector of (point, payload) pairs representing the current Pareto Front.
    ...
    front() const {
      ...
      for ( auto const & e : m_entries )
        if (e.alive)
          out.emplace_back(e.p, e.payload);
      return out;
    }

    /**
     * @brief Checks if the given point $p$ is weakly dominated by any active point in the front.
     *
     * This acts as a quick filter for new points. If $p$ is dominated by the existing front,
     * it cannot be part of the new front, and the expensive dominance checks against $p$ are avoided.
     *
     * @param p The point to check.
     * @return true if $p$ is weakly dominated by an existing point, false otherwise.
     */
    bool
    is_dominated_by_front( Point_type const & p ) const {
      for ( auto const & e : m_entries )
        if ( e.alive && dominates_weakly(e.p, p) )
          return true;
      return false;
    }
     ...
     * @return A pair: {success_of_insertion, unique_id_of_new_entry}. unique_id is 0 if insertion failed.
     *
     * \dot
     * digraph insert {
     * node [shape=box];
     ...
     * PruneFront [label="Soft-delete all E where P strictly dominates E (P \succ E)"];
     * InsertP [label="Add P to m_entries, assign ID"];
     * Rebuild [label="maybe_rebuild()"];
     * Success [label="Return {true, ID}"];
     * Fail [label="Return {false, 0}"];
     *
    ...
    insert( Point_type const & p, Payload_type const & payload = Payload_type{} ) {
      ...
      // 5. Ricostruisci se necessario (solo se abbiamo effettivamente rimosso punti)
    ...
    /**
     * @brief Finds the ID of the alive point in the front that is closest to the given point $p$.
     *
     * The distance metric used is the **squared Euclidean distance** ($L_2^2$ norm):
     * $$d(e, p)^2 = \sum_{i=0}^{N-1} (e.p[i] - p[i])^2$$
     *
     * @param p The reference point.
     * @return A pair: {found\_status, unique\_id\_of\_nearest\_point}. unique\_id is 0 if no alive entry is found.
     */
    std::pair<bool, size_t>
    find_nearest( Point_type const & p ) const {
      ...
      for ( auto const & e : m_entries ) {
        ...
        if (!found || d < bestd) { bestd = d; bestid = e.id; found = true; }
      }
      return {found, bestid};
    }

    /**
     * @brief Builds the Pareto Front from a batch of points, clearing any existing state.
     *
     * This is the preferred method for initial construction or when the entire set
     * of points is available. The algorithm complexity is dominated by the initial sort
     * $O(M \log M)$ where $M$ is the number of points, followed by a sweep $O(M^2)$
     * in the worst-case, but generally much faster in practice, especially for
    ...
    batch_build(Front pts) {
      ...
      for ( auto const & pr : pts ) {
        ...
        // 2a. Check if the candidate is dominated by any point in the current partial front (F)
        ...
        // 2b. Remove points from F that are strictly dominated by the candidate (C)
        for ( size_t j{0}; j < front.size(); ) {
          if (dominates(candidate_point, front[j].p)) {
            // Note: In batch_build, we use physical erase since the vector 'front' is temporary.
    ...
    /// @brief Clears the entire Pareto Front, resetting internal state, entries, and counters.
    void
    clear() {
      m_entries.clear();
      m_tombstones = 0;
     ...
     * @return A pair: {found\_status, payload}. Payload is default-constructed if not found.
     */
