  ...
  private:
        ...
        while (!step_accepted && damping_iter < m_max_damping_iterations) {
          ...
          try {
            ...
            if ( step_accepted ) {
              ...
              // Reduce lambda
              lambda *= m_lambda_factor; ++damping_iter;
                ...
                try {
                    ...
                    if (m_verbose) {
                        fmt::color col = (actual_reduction >= expected_reduction) ?
                                         fmt::color::green : fmt::color::yellow;
                        std::string mark = (actual_reduction >= expected_reduction) ? "✓" : "…";
                        fmt::print(fg(col), "  θ={:.2e} → {:.2e} Δ={:.2e}/{:.2e} {}\n",
                                  theta, norm_f_new, actual_reduction, expected_reduction, mark);
                    }
                ...
                try {
                    ...
                    if (m_verbose) {
                        ...
                        fmt::print(fg(col), "  λ={:.2e} → {:.2e} ΔF={:.2e}/{:.2e} {}\n",
                                  lambda, norm_f_new, actual_reduction, expected_reduction, mark);
                    }
                    ...
                    if (actual_reduction >= expected_reduction) {
                        ...
                        if (actual_reduction > 2.0 * expected_reduction) {
                            lambda = std::min(1.0, lambda / m_griewank_zeta);
                        }
                    } else {
                        // Reduce lambda
            ...
            while (!step_accepted && filter_iter < m_max_damping_iterations) {
                ...
                try {
                    ...
                    for (const auto& [theta_f, F_f] : filter) {
                        if (theta_new >= theta_f - m_filter_gamma_theta * theta_f &&
                            F_new >= F_f - m_filter_gamma_f * F_f) {
                    ...
                    if (m_verbose) {
                        fmt::color col = (acceptable_to_filter && armijo) ?
                                         fmt::color::green : fmt::color::yellow;
                        std::string mark = (acceptable_to_filter && armijo) ? "✓" : "…";
                        fmt::print(fg(col), "  α={:.2e} → θ={:.2e}, F={:.2e} Filter:{}, Armijo:{} {}\n",
                                  alpha, theta_new, F_new,
                                  acceptable_to_filter ? "Y" : "N", armijo ? "Y" : "N", mark);
                    }
                    ...
                    if (acceptable_to_filter && armijo) {
                        ...
                        if (filter.size() > 10) { // Keep filter size manageable
    ...
    solve_cubic_trust_region(NonlinearSystem & system, Vector & x, Vector & f, real_type & norm_f) {
        ...
        for (m_num_iterations = 0; m_num_iterations < m_max_iterations; ++m_num_iterations) {
            ...
            // min m(s) = 0.5||f + J s||^2 + (sigma/3)||s||^3 subject to ||s|| <= delta
            ...
            while (subproblem_iter < 10 && !subproblem_converged) {
                ...
                // Solve (J^T J + sigma * s_norm * I) ds = -J^T (f + J s) - sigma * s_norm * s
                SparseMatrix A = J.transpose() * J;
                for (integer i = 0; i < n; ++i) {
                    A.coeffRef(i, i) += sigma * s_norm;
                }
            ...
            try {
                ...
                if (m_verbose) {
                    fmt::color col = (rho > m_ctr_eta1) ? fmt::color::green :
                                    (rho > 0) ? fmt::color::yellow : fmt::color::red;
                    std::string mark = (rho > m_ctr_eta2) ? "✓✓" :
                                      (rho > m_ctr_eta1) ? "✓" : "✗";
             ...
             try {
                 ...
                 if (m_verbose) {
                     fmt::color col = (rho > m_dogleg_eta1) ? fmt::color::green :
                                     (rho > 0) ? fmt::color::yellow : fmt::color::red;
                     std::string mark = (rho > m_dogleg_eta2) ? "✓✓" :
                                       (rho > m_dogleg_eta1) ? "✓" : "✗";
             ...
             while (ls_iter < m_max_damping_iterations) {
                 ...
                 try {
                     ...
                     if (wolfe1) {
                         ...
                         wolfe2 = (std::abs(new_directional_derivative) <=
                                  m_wolfe_c2 * std::abs(directional_derivative));
                     }
     
                     if (m_verbose) {
                         fmt::color col = (wolfe1 && wolfe2) ? fmt::color::green :
                                         wolfe1 ? fmt::color::yellow : fmt::color::red;
                         std::string mark = (wolfe1 && wolfe2) ? "✓✓" :
                                           wolfe1 ? "✓" : "✗";
                         fmt::print(fg(col), "  α={:.2e} → {:.2e} W1:{}, W2:{} {}\n",
                                   alpha, norm_f_new, wolfe1 ? "Y" : "N", wolfe2 ? "Y" : "N", mark);
                     }
             ...
             try {
                 ...
                 if (m_verbose) {
                     fmt::color col = (rho > m_cubic_eta1) ? fmt::color::green :
                                     (rho > 0) ? fmt::color::yellow : fmt::color::red;
                     std::string mark = (rho > m_cubic_eta2) ? "✓✓" :
                                       (rho > m_cubic_eta1) ? "✓" : "✗";
                 ...
                 if (rho > m_cubic_eta1) {
                     ...
                     if (rho > m_cubic_eta2) {
                         ...
                         sigma = std::max(m_cubic_sigma_min,
                                         m_cubic_gamma_decrease * sigma);
                     }
                 } else {
                     // Unsuccessful step - increase sigma
                     sigma = std::min(m_cubic_sigma_max,
                                     m_cubic_gamma_increase * sigma);
                 }
                 ...
                 try {
                     ...
                     if (m_verbose) {
                         ...
                         fmt::print(fg(col), "  α={:.2e} → {:.2e} Δ={:.2e}/{:.2e} {}\n",
                                   alpha, norm_f_new, actual_reduction, expected_reduction, mark);
                     }
