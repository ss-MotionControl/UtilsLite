...
void AlgoBracket<Real>::set_max_iterations( Integer mit )
{
...
template<typename Real>
void AlgoBracket<Real>::set_max_fun_evaluation( Integer mfev )
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template<typename Real>
Real AlgoBracket<Real>::p_zero2( Real d, Real fd ) const
{
    Real D1{ m_fb - m_fa };
    Real ba{ m_b - m_a };
    Real theta{ ( d - m_a ) / ba };
...
template<typename Real>
Real AlgoBracket<Real>::invp_zero2( Real d, Real fd ) const
{
    //
...
template<typename Real>
Real AlgoBracket<Real>::invp_zero3( Real d, Real fd, Real e, Real fe ) const
{
    //
...
template<typename Real>
Real AlgoBracket<Real>::eval( Real a, Real b )
{
    m_iteration_count = 0;
...
template<typename Real>
Real AlgoBracket<Real>::eval( Real a, Real b, Real amin, Real bmax )
{
    m_iteration_count = 0;
    m_fun_evaluation_count = 0;
...
Real AlgoBracket<Real>::eval()
{
...

    {
        ...
        while ( !( is_finite( fa ) && is_finite( fb ) ) ) {
            ...
            m_converged =
                ( b - a ) < m_tolerance_x || abs_fa < m_tolerance_f || abs_fb < m_tolerance_f;
    ...
    */
    ...
    auto illinois = [this, check]() -> void {
        ...
        while ( ++m_iteration_count < m_max_iteration ) {
            ...
            Real c{ abs_fa < abs_fb ? a + max( tol, fa / ( fa - fb ) ) * ba
    ...
    */
    auto Ridder = [this, check]() -> void {
        /*
           C.F.J.Ridders, “A New Algorithm for Computing a Single Root of a Real Continuous
