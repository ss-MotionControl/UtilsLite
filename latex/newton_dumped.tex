...
\begin{equation}
(\bm{J}^T \bm{J} + \mu \bm{I}) \bm{d} = -\bm{J}^T \bm{f}
\end{equation}
...
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, $\mu_k$, $\Delta_k$
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, $\mu_{k+1}$, $\Delta_{k+1}$
\State Calcola $\bm{d}_k$ risolvendo $(\bm{J}_k^T \bm{J}_k + \mu_k \bm{I}) \bm{d} = -\bm{J}_k^T \bm{f}_k$
\If{$\norm{\bm{d}_k} > \Delta_k$}
    \State $\bm{d}_k = \Delta_k (\bm{d}_k/\norm{\bm{d}_k})$
\EndIf
\State $\bm{x}_{\text{trial}} = \bm{x}_k + \bm{d}_k$
\State Calcola $\bm{f}_{\text{trial}} = \bm{f}(\bm{x}_{\text{trial}})$
\State $\Delta f = \norm{\bm{f}_k} - \norm{\bm{f}_{\text{trial}}}$
\State $\Delta m = 0.5 \bm{d}_k \cdot  (\mu_k \bm{d}_k - \bm{J}_k^T \bm{f}_k)$
...
\begin{equation}
\norm{\bm{f}_k} - \norm{\bm{f}_{\text{trial}}} \geq \beta \theta \norm{\bm{f}_k}
\end{equation}
...
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, $\theta_k$
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, $\theta_{k+1}$
\State Calcola $\bm{d}_k = -\bm{J}_k^{-1} \bm{f}_k$
\State $\theta = \theta_k$
\For{$j = 1$ to $j_{\max}$}
    \State $\bm{x}_{\text{trial}} = \bm{x}_k + \theta \bm{d}_k$
    \State Calcola $\bm{f}_{\text{trial}} = \bm{f}(x_{\text{trial}})$
    \State $\Delta = \norm{\bm{f}_k} - \norm{\bm{f}_{\text{trial}}}$
...
La condizione di accettazione è:
\begin{equation}
F_k - F_{\text{trial}} \geq -\eta (\bm{g}_k^T \bm{d}_k)
\end{equation}
dove $\bm{g}_k = \bm{J}_k^T \bm{f}_k$ è il gradiente di $F$.

\subsubsection{Algoritmo}
\begin{algorithm}[H]
\caption{Griewank (1980)}
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, $F_k = \frac{1}{2}\norm{\bm{f}_k}^2$
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, $F_{k+1}$
\State Calcola $\bm{g}_k = \bm{J}_k^T \bm{f}_k$
\State Calcola $\bm{d}_k = -\bm{J}_k^{-1} \bm{f}_k$
\State $\lambda = 1.0$
\For{$j = 1$ to $j_{\max}$}
    \State $\bm{x}_{\text{trial}} = \bm{x}_k + \lambda \bm{d}_k$
    \State Calcola $\bm{f}_{\text{trial}} = \bm{f}(\bm{x}_{\text{trial}})$
    \State $F_{\text{trial}} = \frac{1}{2}\norm{\bm{f}_{\text{trial}}}^2$
    \State $\Delta F = F_k - F_{\text{trial}}$
    \State $\Delta F_{\text{atteso}} = -\eta (\bm{g}_k^T \bm{d}_k)$
...
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, filter
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, filter aggiornato
\State Calcola $\bm{d}_k = -\bm{J}_k^{-1} \bm{f}_k$
\State $\alpha = 1.0$
\For{$j = 1$ to $j_{\max}$}
    \State $\bm{x}_{\text{trial}} = \bm{x}_k + \alpha \bm{d}_k$
    \State Calcola $\bm{f}_{\text{trial}} = \bm{f}(\bm{x}_{\text{trial}})$
    \State $\theta_{\text{trial}} = \norm{\bm{f}_{\text{trial}}}$
    \State $F_{\text{trial}} = \frac{1}{2}\theta_{\text{trial}}^2$
    \State Verifica accettabilità rispetto al filter
    \State Verifica condizione di Armijo: $F_{\text{trial}} \leq F_k + \alpha c_1 (\bm{g}_k^T \bm{d}_k)$
...
\begin{equation}
\min_{s} m(s) = \frac{1}{2}\norm{\bm{f} + \bm{J} \bm{s}}^2 + \frac{\sigma}{3}\norm{\bm{s}}^3 \quad \text{s.t.} \quad \norm{\bm{s}} \leq \Delta
\end{equation}
...
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, $\Delta_k$, $\sigma_k$
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, $\Delta_{k+1}$, $\sigma_{k+1}$
\State Calcola $\bm{g}_k = \bm{J}_k^T \bm{f}_k$
\State Risolvi sottoproblema cubico per $\bm{s}_k$
\State $\bm{x}_{\text{trial}} = \bm{x}_k + \bm{s}_k$
\State Calcola $\bm{f}_{\text{trial}} = \bm{f}(\bm{x}_{\text{trial}})$
\State $\rho = \dfrac{F_k - F_{\text{trial}}}{m(0) - m(\bm{s}_k)}$
\If{$\rho > \eta_1$}
    \State Accetta passo
    \If{$\rho > \eta_2$}
...
\begin{itemize}
    \item Direzione di Cauchy: $\bm{d}_C = -\alpha \bm{g}$ dove $\alpha = \frac{\norm{\bm{g}}^2}{\norm{\bm{J} \bm{g}}^2}$
    \item Direzione di Gauss-Newton: $\bm{d}_{GN} = -\bm{J}^{-1} \bm{f}$
\end{itemize}
...
\bm{d}(\tau) = 
\begin{cases}
\tau \bm{d}_C & \text{se } \tau \in [0, 1] \\
\bm{d}_C + (\tau-1)(\bm{d}_{GN} - \bm{d}_C) & \text{se } \tau \in [1, 2]
\end{cases}
\end{equation}
...
\begin{algorithmic}[1]
\Require $\bm{x}_k$, $\bm{f}_k$, $\norm{\bm{f}_k}$, $\bm{J}_k$, $\Delta_k$
\Ensure $\bm{x}_{k+1}$, $\bm{f}_{k+1}$, $\norm{\bm{f}_{k+1}}$, $\Delta_{k+1}$
\State Calcola $\bm{g}_k = \bm{J}_k^T \bm{f}_k$
\State Calcola $\bm{d}_C = -\alpha \bm{g}_k$
\State Calcola $\bm{d}_{GN} = -\bm{J}_k^{-1} \bm{f}_k$
\If{$\norm{\bm{d}_{GN}} \leq \Delta_k$}
    \State $\bm{d}_k = \bm{d}_{GN}$
\ElsIf{$\norm{\bm{d}_C} \geq \Delta_k$}
    \State $\bm{d}_k = (\Delta_k / \norm{\bm{d}_C}) \bm{d}_C$
\Else
    \State Calcola punto su Dogleg tra $\bm{d}_C$ e $\bm{d}_{GN}$ con $\norm{\bm{d}} = \Delta_k$
\EndIf
...
\begin{enumerate}
    ...
    \begin{equation}
    F(\bm{x} + \alpha \bm{d}) \leq F(\bm{x}) + c_1 \alpha \bm{g}^T \bm{d}
    \end{equation}
    
    \item Condizione di curvatura:
    \begin{equation}
    \abs{\bm{g}_{\text{new}}^T \bm{d}} \leq c_2 \abs{\bm{g}^T \bm{d}}
    \end{equation}
\end{enumerate}
