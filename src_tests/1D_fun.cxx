...
build_1dfun_list( std::vector<std::unique_ptr<fun1D>> & f_list )
{
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>(
      new fun1D( 0, 6, "f(x) = |x-5|(x-5)", []( real_type x ) -> real_type { return abs( x - 5 ) * ( x - 5 ); } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -1, 2, "f(x) = x^9",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type x2{ x * x };
                                                            real_type x4{ x2 * x2 };
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -1, 1, "f(x) = 1/3+sign(x)|x|^(1/3)+x^3",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type s{ real_type( x > 0 ? 1 : -1 ) };
                                                            return 1.0 / 3.0 + power3( x ) +
                                                                   s * pow( abs( x ), 1.0 / 3.0 );
                                                          } ) ) );
  ...
  for ( int n : { 1, 5, 15, 20, 200 } )
    f_list.emplace_back( std::unique_ptr<fun1D>(
        new fun1D( 0, 1, fmt::format( "f(x) = 2 * x * exp( -n ) - 2 * exp( -n * x )+1, n={}", n ),
                   [n]( real_type x ) -> real_type { return 2 * x * exp( -n ) - 2 * exp( -n * x ) + 1; } ) ) );

  for ( int n : { 2, 5, 15, 20, 200 } )
    f_list.emplace_back( std::unique_ptr<fun1D>(
        new fun1D( 0, 1, fmt::format( "f(x) = ( 1 + (1-n)^2 ) * x - ( 1 - n * x )^2, n={}", n ),
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( 0.5, 8, "f(x) = (10-x)*exp(-10*x)-pow(x,10)+1", []( real_type x ) -> real_type
                                         { return ( 10 - x ) * exp( -10 * x ) - pow( x, 10 ) + 1; } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 1, 4, "f(x) = exp(sin(x))-x-1", []( real_type x ) -> real_type
                                                          { return exp( sin( x ) ) - x - 1; } ) ) );
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( 0, 1, "f(x) = x^2+sin(x/10)-1/4", []( real_type x ) -> real_type
                                         { return power2( x ) + sin( x / 10 ) - 0.25; } ) ) );
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( 2.6, 3.5, "f(x) = exp(x^2+7*x-30)-1",
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( 0.0, 1.0, "f(x) = tan( pi * ( x^8 - 1/2 ) )", []( real_type x ) -> real_type
                                         { return tan( m_pi * ( power8( x ) - 0.5 ) ); } ) ) );
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( 0, 0.5, "f(x) = exp(-x)-x-sin(x)",
                                         []( real_type x ) -> real_type { return exp( -x ) - x - sin( x ); } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>(
      new fun1D( 0.1, 1.5, "f(x) = x^3-1", []( real_type x ) -> real_type { return power3( x ) - 1; } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -1, 2, "f(x) = x^2-sin(x)^2-1", []( real_type x ) -> real_type
                                                          { return power2( x ) - power2( sin( x ) ) - 1; } ) ) );
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( -1.0, 1.0, "f(x) = x > 0 ? 1/(1-x) : x-1",
                                         []( real_type x ) -> real_type { return x > 0 ? 1 / ( 1 - x ) : x - 1; } ) ) );

  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( m_pi / 2, m_pi, "f(x) = sin(x) - x/2",
                                         []( real_type x ) -> real_type { return sin( x ) - x / 2; } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>(
      new fun1D( 0.0, 1.0, "f(x) = x * exp(x) - 1", []( real_type x ) -> real_type { return x * exp( x ) - 1; } ) ) );

  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -9, 31, "f(x) = a * x * exp( b * x ), a=-40, b=-1",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type a{ -40 }, b{ -1 };
                                                            return a * x * exp( b * x );
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -9, 31, "f(x) = a * x * exp( b * x ), a=-100, b=-2",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type a{ -100 }, b{ -2 };
                                                            return a * x * exp( b * x );
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -9, 31, "f(x) = a * x * exp( b * x ), a=-200, b=-3",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type a{ -200 }, b{ -3 };
                                                            return a * x * exp( b * x );
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=4, a=0.2",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 4 }, a{ 0.2 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=6, a=0.2",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 6 }, a{ 0.2 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=8, a=0.2",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 8 }, a{ 0.2 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=10, a=0.2",
                                                          []( real_type x ) -> real_type
                                                          {
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=12, a=0.2",
                                                          []( real_type x ) -> real_type
                                                          {
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=8, a=1",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 8 }, a{ 1 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=10, a=1",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 10 }, a{ 1 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=12, a=1",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 12 }, a{ 1 };
                                                            return pow( x, n ) - a;
  ...
  f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 5, "f(x) = x^n-a, n=14, a=1",
                                                          []( real_type x ) -> real_type
                                                          {
                                                            real_type n{ 14 }, a{ 1 };
                                                            return pow( x, n ) - a;
  ...
  for ( int n : { 2, 5, 10, 15, 20 } )
    f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 1, fmt::format( "f(x) = x^2-(1-x)^n, n={}", n ),
                                                            [n]( real_type x ) -> real_type
                                                            { return power2( x ) - pow( 1 - x, n ); } ) ) );

  for ( int n : { 1, 2, 3, 5, 8, 15, 20 } )
    f_list.emplace_back( std::unique_ptr<fun1D>(
        new fun1D( 0, 1, fmt::format( "f(x) = (1+(1-n)^4)*x-(1-n*x)^4, n={}", n ),
                   [n]( real_type x ) -> real_type { return ( 1 + power4( 1 - n ) ) * x - power4( 1 - n * x ); } ) ) );

  for ( int n : { 1, 5, 10, 15, 20 } )
    f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 1, fmt::format( "f(x) = exp(-n*x)*(x-1)+x^n, n={}", n ),
                                                            [n]( real_type x ) -> real_type
                                                            { return exp( -n * x ) * ( x - 1 ) + pow( x, n ); } ) ) );

  for ( int n : { 2, 5, 10, 15, 20 } )
    f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0.01, 1, fmt::format( "f(x) = (n*x-1)/((n-1)*x), n={}", n ),
                                                            [n]( real_type x ) -> real_type
                                                            { return ( n * x - 1 ) / ( ( n - 1 ) * x ); } ) ) );

  for ( int n : { 2, 3, 6, 9, 11, 15, 20, 25, 33 } )
    f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( 0, 100, fmt::format( "f(x) = x^(1/n)-n^(1/n), n={}", n ),
                                                            [n]( real_type x ) -> real_type
                                                            {
  ...
  f_list.emplace_back(
      std::unique_ptr<fun1D>( new fun1D( -1, 4, "f(x) = x == 0 ? 0 : x*exp(-1/x^2)", []( real_type x ) -> real_type
                                         { return x == 0 ? 0 : x * exp( -1 / ( x * x ) ); } ) ) );

  for ( int n : { 1, 2, 3, 4, 5, 6, 7, 8, 10, 20, 30, 40 } )
    f_list.emplace_back( std::unique_ptr<fun1D>(
        new fun1D( -1e4, m_pi / 2, fmt::format( "f(x) = x < 0 ? -n/20 : (n/20)*(x/1.5+sin(x)-1), n={}", n ),
                   [n]( real_type x ) -> real_type
                   {
  ...
  for ( int n : { 20, 30, 40, 100, 200, 400, 600, 800, 1000 } )
    f_list.emplace_back( std::unique_ptr<fun1D>(
        new fun1D( -1e4, 1e-4, fmt::format( "f(x) = [exp(1)-1.859,-0.859,exp( (n+1)*0.5e3*x )-1.859], n={}", n ),
                   [n]( real_type x ) -> real_type
                   {
  ...
  for ( real_type RHS : { -229.970950036057, 0.0, 10.0 } )
    f_list.emplace_back( std::unique_ptr<fun1D>( new fun1D( -100, 100, fmt::format( "f(x) = penalty(x) RHS={}", RHS ),
                                                            [RHS]( real_type x_in ) -> real_type
                                                            {
                                                              real_type m_h       = 0.01;
                                                              real_type m_epsilon = 0.01;
                                                              real_type m_A       = 1 / m_h;
                                                              real_type m_A1      = ( 1 - m_epsilon ) *
                                                                               power2( m_h / ( 1 - m_h ) );
