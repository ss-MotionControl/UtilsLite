...
#include "Utils_DifferentialEvolution.hh"
...
print_summary_table(const vector<DETestResult>& results) {
  ...
  constexpr integer total_width = 2 + col_idx + 3 + col_status + 3 + col_neq + 3 +
                                  col_pop + 3 + col_iter + 3 + col_feval + 3 +
                                  col_fitness + 3 + col_residual + 3 + col_time + 3 +
                                  col_name + 2;
  
  // Intestazione della tabella
  fmt::print("\n\n");
  fmt::print(fg(fmt::color::cyan) | fmt::emphasis::bold, "{:━^{}}\n", 
            " DIFFERENTIAL EVOLUTION TEST RESULTS ", total_width);
...
void print_usage(const char* prog_name) {
  ...
  fmt::print("  --max-iter=N           Set maximum number of DE iterations (integer)\n");
  fmt::print("  --tolerance=VAL        Set stopping tolerance (floating, e.g. 1e-8)\n");
  fmt::print("  --pop-size=N           Set population size (integer, default: 10*Dim)\n");
...
main(int argc, char* argv[]) {
  ...
  if (custom_pop_size) {
    fmt::print(fg(fmt::color::white), "  Population size: {}\n", custom_pop_value);
  } else {
  ...
  if (!all_results.empty()) {
    ...
    auto worst_it = max_element(all_results.begin(), all_results.end(),
                               [](const DETestResult& a, const DETestResult& b) {
                                 return a.converged ? a.best_fitness : -1e100 < 
                                        b.converged ? b.best_fitness : -1e100;
                               });
