...
print_line_search_statistics()
{
  fmt::print( fmt::fg( fmt::color::light_blue ),
              ...
              "╠══════════════════╤══════════╤══════════╤════════════╤═════════"
              "════╤══════════════════════╣\n"
              "║    LineSearch    │ Tests    │ Success  │  Success % │   "
              "AvgIter   │     AvgFuncEval      ║\n"
              "╠══════════════════╪══════════╪══════════╪════════════╪═════════"
              "════╪══════════════════════╣\n" );
  ...
  for ( const auto & [name, stats] : line_search_statistics )
  {
    ...
    Scalar avg_func_evals = ( stats.successful_tests > 0 )
                                ? static_cast<Scalar>( stats.total_evaluations ) / stats.successful_tests
                                : 0.0;
...
print_summary_table()
{
  ...
  for ( auto const & result : global_test_results )
  {
    ...
    fmt::print( "║ {:<22} │ {:>6} │ {:<12} │ {:>8} │ {:<14.4e} │ ", problem_name, result.dimension,
                result.linesearch_name, result.result_data.total_iterations, result.result_data.final_function_value );
...
test( OptimizationProblem<T, N> const * tp, string const & problem_name )
{
  ...
  vector<pair<string, std::function<std::optional<
                          std::tuple<Scalar, size_t>>( Scalar, Scalar, Vector const &, Vector const &,
                                                       std::function<Scalar( Vector const &, Vector * )>, Scalar )>>>
      line_searches;
  ...
  line_searches.emplace_back( "Armijo",
                              [&armijo]( Scalar f0, Scalar Df0, Vector const & x, Vector const & d,
  ...
  for ( const auto & [ls_name, line_search] : line_searches )
  {
    ...
    fmt::print( "{} - {}: {} after {} iterations, f = {:.6e}\n", problem_name, ls_name, status_str,
                solution_data.total_iterations, solution_data.final_function_value );
