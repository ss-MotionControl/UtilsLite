...
class BlockMult
{
  ...
  // (n x m) * (m x p)
  void Compute_C_block( mat const & A, mat const & B, mat & C, integer i, integer j ) const;
...
void
BlockMult::Compute_C_block( mat const & A, mat const & B, mat & C, integer const i, integer const j ) const
{
  ...
  std::vector<integer> const & k_block )
{
  if ( A.cols() != B.rows() )
  {
    fmt::print( "Invalid matrix multiplication. Found {} x {} Times {} x {} ", A.cols(), A.rows(), B.cols(), B.rows() );
    return false;
  }
...
main()
{
  ...
  i_block.clear();
  i_block.reserve( n + 1 );
  i_block.emplace_back( 0 );
  k_block.clear();
  k_block.reserve( p + 1 );
  k_block.emplace_back( 0 );
  j_block.clear();
  j_block.reserve( m + 1 );
  j_block.emplace_back( 0 );
  ...
  for ( int nptp{ 0 }; nptp <= 5; ++nptp )
  {
    ...
    {
      case 0:
        name = Utils::ThreadPool0::Name();
        break;
      case 1:
        name = Utils::ThreadPool1::Name();
        break;
      case 2:
        name = Utils::ThreadPool2::Name();
        break;
      case 3:
        name = Utils::ThreadPool3::Name();
        break;
      case 4:
        name = Utils::ThreadPool4::Name();
        break;
      case 5:
        name = Utils::ThreadPool5::Name();
        break;
    }
