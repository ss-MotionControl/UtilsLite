...
static inline real_type
power3( real_type const x )
{
  return x * x * x;
}
static inline real_type
power4( real_type const x )
{
  return power2( power2( x ) );
}
static inline real_type
power5( real_type const x )
{
  return power4( x ) * x;
}
...
public:
  ...
  real_type
  D( real_type const x ) const override
  {
    real_type res{ 0 };
...
public:
  ...
  eval( real_type const x ) const override
  {
    return x * exp( -n ) - exp( -n * x ) + 0.5;
  }
  real_type
  D( real_type const x ) const override
  {
    return exp( -n ) + n * exp( -n * x );
  }
};
...
public:
  ...
  D( real_type const x ) const override
  {
    return 1 + power4( 1 - n ) + 4 * power3( 1 - n * x ) * n;
  }
};
...
static void
do_solve( real_type const a, real_type const b, Utils::AlgoHNewton_base_fun<real_type> const * f )
{
...
{
  {
    ...
    public:
      ...
      eval( real_type const x ) const override
      {
        return x > 0 ? 1 / ( 1 - x ) : x - 1;
      }
      real_type
      D( real_type const x ) const override
      {
        return x > 0 ? 1 / power2( 1 - x ) : 1;
      }
    };
    FUN f;
    do_solve( -1.0, 1.0, &f );
  ...
  for ( int i = 1; i <= 10; ++i ) do_solve( power2( i ) + 1e-9, power2( i + 1 ) - 1e-9, &fun1 );
  ...
  }
  {
    ...
    public:
      ...
      D( real_type const x ) const override
      {
        return ( 10 * x - 101 ) * exp( -10 * x ) - 10 * pow( x, 9 );
      }
    };
    FUN f;
    do_solve( 0.5, 8, &f );
