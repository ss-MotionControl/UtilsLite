...
/**
 * @class Rosenbrock2D
 * @brief 2D Rosenbrock function, a classic benchmark for optimization algorithms
 * 
...
public:
   ...
   */
  Vector exact() const { Vector x(2); x << 1.0, 1.0; return x; }
...
public:
   ...
   */
  Vector exact() const { return Vector::Constant(N, 1.0); }
 ...
 * The function is defined as: f(x) = Σ[i=0 to N-2] [(1-x_i)² + 100(x_{i+1}-x_i²)²]
 */
template <typename T, int N>
class RosenbrockN {
public:
...
public:
   ...
   */
  Vector exact() const { return Vector::Zero(N); }
...
public:
   ...
   */
  Vector exact() const { Vector x(2); x << 3.0, 0.5; return x; }
...
public:
   ...
   */
  Vector exact() const { Vector x(2); x << 3.0, 2.0; return x; }
...
public:
   ...
   */
  Vector exact() const { Vector x(2); x << 5.0, 4.0; return x; }
...
public:
   ...
   */
  Vector exact() const { Vector x(3); x << 1.0, 0.0, 0.0; return x; }
 ...
 * 
 * This function has a global minimum when the product of all variables equals 1.
 * The function is designed to test algorithms on problems with nearly linear constraints.
 */
template <typename T, int N>
class BrownAlmostLinearN {
public:
 ...
 * algorithms on problems with banded structure. The global minimum is at (1,1,...,1).
 */
template <typename T, int N>
class BroydenTridiagonalN {
public:
...
public:
   ...
   */
  Vector lower() const { return Vector::Constant(N, -500.0); }
  
  /**
   * @brief Get upper bounds for variables
   * @return Vector of upper bounds (500.0 for all variables)
   */
  Vector upper() const { return Vector::Constant(N, 500.0); }
   ...
   */
  Vector exact() const { return Vector::Constant(N, 420.9687); }
  ...
  T operator()(Vector const & x) const {
    ...
    for (int i = 0; i < N; ++i) {
      f -= x[i] * std::sin(std::sqrt(std::abs(x[i])));
    }
    return f;
  }
};
...
public:
   ...
   */
  Vector lower() const { return Vector::Constant(N, -32.768); }
  
  /**
   * @brief Get upper bounds for variables
   * @return Vector of upper bounds (32.768 for all variables)
   */
  Vector upper() const { return Vector::Constant(N, 32.768); }
...
public:
   ...
   */
  Vector lower() const { return Vector::Constant(N, -5.12); }
  
  /**
   * @brief Get upper bounds for variables
   * @return Vector of upper bounds (5.12 for all variables)
   */
  Vector upper() const { return Vector::Constant(N, 5.12); }
